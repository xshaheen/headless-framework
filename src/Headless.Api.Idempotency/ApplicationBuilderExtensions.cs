...
public static class IdempotencyApplicationBuilderExtensions
{
    extension(IApplicationBuilder app)
    {
        ...
        /// <c>UseHeadlessTenancy()</c>. The durable store keys every record by the current tenant
        /// and the middleware scopes keys by the current user; tenant and auth must be resolved first
        /// so unauthenticated and unauthorized requests don't allocate idempotency storage.
        /// </remarks>
