...
/// <summary>
/// Extension members for selecting Infobip as the default (unkeyed) SMS provider on
/// <see cref="HeadlessSmsSetupBuilder"/>. Named instances are configured through
/// <see cref="SetupInfobipNamed"/>.
/// </summary>
[PublicAPI]
public static class SetupInfobip
{
        ...
        )
        {
            ...
            setup.RegisterDefaultProvider(services =>
                AddInfobipSmsCore(
                    services,
                    name: null,
                    (s, n) => s.Configure<InfobipSmsOptions, InfobipSmsOptionsValidator>(config, n),
                    configureClient,
                    configureResilience
                )
            );
        ...
        )
        {
            ...
            setup.RegisterDefaultProvider(services =>
                AddInfobipSmsCore(
                    services,
                    name: null,
                    (s, n) => s.Configure<InfobipSmsOptions, InfobipSmsOptionsValidator>(setupAction, n),
                    configureClient,
                    configureResilience
                )
            );
    ...
    /// <summary>
    /// The Infobip HttpClient name doubles as the resilience-pipeline key, so each named instance gets its
    ...
    /// <summary>
    /// Registers the Infobip SMS sender. <paramref name="name"/> <see langword="null"/> registers the default
    ...
    /// </summary>
    internal static void AddInfobipSmsCore(
    ...
    )
    {
        ...
        if (name is null)
        {
            services.AddSingleton<ISmsSender>(static sp => new InfobipSmsSender(
                sp.GetRequiredService<IHttpClientFactory>(),
                HttpClientName,
                sp.GetRequiredService<IOptionsMonitor<InfobipSmsOptions>>(),
                optionsName: null,
                sp.GetRequiredService<ILogger<InfobipSmsSender>>()
            ));
...
/// <summary>
/// Extension members for selecting Infobip for a named SMS instance on
...
/// </summary>
[PublicAPI]
public static class SetupInfobipNamed
{
    extension(HeadlessSmsInstanceBuilder instance)
    {
        /// <summary>Uses Infobip for this named instance, binding and validating <see cref="InfobipSmsOptions"/> from configuration.</summary>
        /// <remarks>
        /// HTTP retry is disabled by default because SMS sends are not idempotent. Pass
        ...
        /// <exception cref="ArgumentNullException"><paramref name="config"/> is <see langword="null"/>.</exception>
        public HeadlessSmsInstanceBuilder UseInfobip(
            IConfiguration config,
            Action<HttpClient>? configureClient = null,
            Action<HttpStandardResilienceOptions>? configureResilience = null
        )
        {
            ...
            instance.RegisterProvider(services =>
                SetupInfobip.AddInfobipSmsCore(
                    services,
                    name,
                    (s, n) => s.Configure<InfobipSmsOptions, InfobipSmsOptionsValidator>(config, n),
                    configureClient,
        ...
        /// <summary>Uses Infobip for this named instance, configuring <see cref="InfobipSmsOptions"/> via a delegate.</summary>
        /// <param name="setupAction">Delegate that populates the options.</param>
        /// <param name="configureClient">Optional delegate to further configure the underlying <see cref="HttpClient"/>.</param>
        /// <param name="configureResilience">Optional delegate to override the default resilience pipeline.</param>
        /// <returns>The instance builder, for chaining.</returns>
        ...
        /// <summary>Uses Infobip for this named instance, configuring <see cref="InfobipSmsOptions"/> with access to the service provider.</summary>
        /// <param name="setupAction">Delegate that populates the options, with access to the resolved service provider.</param>
        /// <param name="configureClient">Optional delegate to further configure the underlying <see cref="HttpClient"/>.</param>
        /// <param name="configureResilience">Optional delegate to override the default resilience pipeline.</param>
        /// <returns>The instance builder, for chaining.</returns>
