...
public static class AddCacheExtensions
{
    extension(IServiceCollection services)
    {
        ...
        private IServiceCollection _AddCacheCore(bool isDefault)
        {
            ...
            if (!isDefault)
            {
                services.AddKeyedSingleton<ICache>(CacheConstants.MemoryCacheProvider, provider => provider.GetRequiredService<IInMemoryCache>());
            }
            else
            {
                ...
                services.TryAddSingleton<IDistributedCache>(provider => new InMemoryCacheDistributedCacheAdapter(provider.GetRequiredService<IInMemoryCache>()));
                services.TryAddSingleton(typeof(IDistributedCache<>), typeof(DistributedCache<>));
                services.AddKeyedSingleton(CacheConstants.DistributedCacheProvider, x => x.GetRequiredService<IDistributedCache>());
