...
> **API note.** `PublishOptions` is a sealed record and `ConsumeContext<TMessage>` is a non-sealed record class over the non-generic `ConsumeContext` base. They support value-style `with` expressions for non-destructive mutation. Publish middleware can stamp a tenant with `context.Options = (context.Options ?? new PublishOptions()) with { TenantId = "acme" }` without manually copying every other field. Note that `Headers` carries an `IDictionary<string,string?>`, whose default reference-equality may surprise callers comparing two instances that differ only in header contents.
