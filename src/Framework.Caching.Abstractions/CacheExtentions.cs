...
/// <summary>
/// Extension methods for <see cref="ICache"/> with cache stampede protection.
/// </summary>
[PublicAPI]
public static class CacheExtensions
{
    private static readonly ConditionalWeakTable<ICache, KeyedLock> _Locks = new();
    ...
    /// </list>
    /// <para>
    /// Each <see cref="ICache"/> instance maintains its own lock pool, so different
    /// cache instances can execute factories concurrently for the same key.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code>
    ...
    )
    {
        ...
        var cacheValue = await cache.GetAsync<T>(key, cancellationToken).ConfigureAwait(false);
        ...
        var locks = _Locks.GetOrCreateValue(cache);
        ...
        using (await locks.LockAsync(key, cancellationToken).ConfigureAwait(false))
        {
            ...
            cacheValue = await cache.GetAsync<T>(key, cancellationToken).ConfigureAwait(false);
...
{
    private static readonly ConditionalWeakTable<object, KeyedLock> _Locks = new();
    ...
    )
    {
        ...
        // First check - avoid lock acquisition if already cached
        var cacheValue = await cache.GetAsync(key, cancellationToken).ConfigureAwait(false);
        ...
        // Acquire per-key lock for stampede protection
        // Use object-typed ConditionalWeakTable to handle ICache<T> variance
        var locks = _Locks.GetOrCreateValue(cache);

        using (await locks.LockAsync(key, cancellationToken).ConfigureAwait(false))
        {
            // Double-check after acquiring lock
            cacheValue = await cache.GetAsync(key, cancellationToken).ConfigureAwait(false);
