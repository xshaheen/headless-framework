namespace Headless.Ticker.SourceGenerator.Validation;
...
public static class CronValidator
{
    ...
    public static bool IsValidCronExpression(string expression)
    {
        ...
        var parts = new string[_RequiredPartsCount];
        var partCount = _SplitIntoSpan(expressionSpan, parts);

        // Only support 6-part cron expressions with seconds
        if (partCount != _RequiredPartsCount)
        {
            return false;
        }
    ...
    /// </summary>
    private static int _SplitIntoSpan(ReadOnlySpan<char> input, Span<string> output)
    {
        var partIndex = 0;
        var start = 0;
    ...
    private static bool _ValidatePart(string part, int min, int max)
    {
