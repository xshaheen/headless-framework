...
- **Scheduling options**: `JobOptions` and `RecurringJobOptions` map description, durable retry count/intervals, node-death policy, and the transient `RequireAtomicEnlistment` requirement; recurring options also accept a nullable IANA `TimeZoneId`. Priority remains immutable `[JobFunction]` / descriptor metadata.
...
The host `ConfigureDefaults` atomic-enlistment requirement applies only to one-shot jobs, including keyed jobs and chain nodes. Recurring definitions ignore that host default while inheriting retry and node-death defaults. An explicit `RequireAtomicEnlistment` policy configured by request type or descriptor, or `RecurringJobOptions.RequireAtomicEnlistment` on the call, requires the definition write to enlist in a compatible live relational transaction: missing or incompatible coordination fails before persistence, and a compatible transaction carries the definition row (and its store-anchored schedule position) to commit or rollback with the caller. Neither one-shot nor recurring calls can weaken a host or function requirement. Startup seeding of attribute-defined cron definitions (`[JobFunction]` cron expressions, through `IInternalJobManager`) does not consult these policies: it runs before any application transaction exists and definitions are idempotent, so it stays exempt.

Set `JobOptions.RequireAtomicEnlistment = true` for an ordinary or keyed one-shot deadline, or `RecurringJobOptions.RequireAtomicEnlistment = true` for a recurring definition, that must commit with application state. Low-level callers can set the transient `TimeJobEntity.RequireAtomicEnlistment` / `CronJobEntity.RequireAtomicEnlistment`; both are excluded from persisted columns and JSON, and the time-job flag from intent fingerprints too. Required calls reject missing/nonrelational/incompatible coordination before scheduling middleware. Direct persistence cannot satisfy this assertion. The default remains automatic: a compatible ambient transaction enlists the write, otherwise scheduling uses the existing direct path. For chains, the existing `JobOptions` on `Start`, `Then`, and `Catch` carry the same assertion: if any node requires atomic enlistment, the whole tree must enlist before any scheduling middleware runs.

The guarantee, by requirement and coordination state, is the Jobs counterpart of the Messaging delivery-mode matrix (`RequireAtomicEnlistment` behaves like `DeliveryMode.Coordinated`; see [Delivery Modes](../../docs/llms/messaging.md#delivery-modes)). Every throw happens before scheduling middleware, persistence, or side effects:

| `RequireAtomicEnlistment` | Compatible live relational transaction | No coordinated scope, or a scope with no relational transaction | Relational transaction that is dead or belongs to another database |
|---|---|---|---|
| unset (default) | enlist the row in the caller's transaction, side effects after commit | insert directly, side effects immediately | throw |
| `true` (call or function policy) | enlist the row in the caller's transaction, side effects after commit | throw | throw |

A coordinated scope that carries no relational transaction — for example a messaging-only or in-memory scope — is not infectious: the default inserts directly instead of failing, while a required call rejects it. A relational transaction that is present but already completed, or that belongs to another database, throws for every call, because the caller opened a transaction expecting atomicity and a silent fallback would reintroduce the divergence this feature prevents.

Two coordinator behaviors are documented, not defects. Commit callbacks are savepoint-blind: the post-commit signal registered for a job row written inside a savepoint that is later rolled back still runs when the outer transaction commits; for Jobs this is harmless, because the worker re-reads due rows from the store and finds nothing. Under EF's execution strategy, the plain-`DbContext` `ExecuteCoordinatedTransactionAsync` helper replays the whole operation, job writes included, for a failure before `CommitAsync` starts; the Headless save pipeline (`HeadlessDbContext`) honors the `CommitRetryGuard` that every coordinated Jobs write sets and does not replay. See [Commit Coordination](../../docs/llms/commit-coordination.md#quick-orientation).
