...
Relational consumers must apply the Jobs migrations before deploying the new binary, including cancellation; cron pause/time-zone fields; and `ReconciledThroughUtc`, `NextDueUtc`, `MissedRunGraceSeconds`, `OnMissedRun`, nullable `EvaluationFingerprint`, and nullable occurrence `RecoveredFromUtc`. Legacy positions backfill to the uninitialized sentinel, so the first new scheduler wake anchors them at store time instead of replaying history. Quiesce every scheduler node during migration and do not run mixed old/new schedulers: old code can materialize outside the new position transaction. The PostgreSQL demos and SQL Server conformance project include reference migrations; custom stores own equivalent DDL and indexes. Refuse rollback once watermark or recovery state exists unless that state is deliberately exported or discarded. Custom `IJobPersistenceProvider` implementations must implement atomic pause/resume/update plus the new `GetEarliestCronDispatchCandidatesAsync`, `AdvanceCronScheduleAsync`, and `MaterializeCronScheduleOccurrenceAsync` members before upgrading. Selection must be uncached and return the store-time snapshot; position-only advance returns `null` on a lost fence; due dispatch must atomically materialize or recognize the exact persisted projection and advance its position. Run the shared schedule-position conformance scenarios against every custom provider.
