...
/// <summary>
/// Single-call coordinated-transaction helpers for <see cref="HeadlessDbContext"/>: open a resilient
/// transaction, enlist it in commit coordination, run the operation, and commit — so deferred work
/// (outbox dispatch, durable jobs) buffered inside the operation drains atomically on commit and is
/// discarded on rollback. The enlist cannot be forgotten because it is welded into the helper.
/// </summary>
/// <remarks>
/// <para>
...
public static class HeadlessCoordinatedTransactionExtensions
{
    ...
    /// </summary>
    /// <param name="context">The <see cref="HeadlessDbContext"/> to operate on.</param>
