...
using Framework.Ticker.Exceptions;
...
)
{
    ...
    )
    {
        ...
        using var jobActivity = tickerQInstrumentation.StartJobActivity(
            $"Framework.Ticker.job.execute.{context.Type.ToString().ToLowerInvariant()}",
            context
        );

        // Add additional tags to the activity
        jobActivity?.SetTag("Framework.Ticker.job.is_due", isDue);
        jobActivity?.SetTag("Framework.Ticker.job.is_child", isChild);
        ...
        for (var attempt = context.RetryCount; attempt <= context.Retries; attempt++)
        {
            ...
            jobActivity?.SetTag("Framework.Ticker.job.current_attempt", attempt + 1);
            ...
            catch (TaskCanceledException ex)
            {
                ...
                jobActivity?.SetTag("Framework.Ticker.job.final_status", context.Status.ToString());
                jobActivity?.SetTag("Framework.Ticker.job.cancellation_reason", "Task was cancelled");
            ...
            catch (TerminateExecutionException ex)
            {
                ...
                if (ex.InnerException != null)
                {
                    ...
                    jobActivity?.SetTag("Framework.Ticker.job.skip_reason", ex.InnerException.Message);
                }
                else
                {
                    context.SetProperty(x => x.ExceptionDetails, ex.Message);
                    jobActivity?.SetTag("Framework.Ticker.job.skip_reason", ex.Message);
                }
        ...
        if (success)
        {
            ...
            jobActivity?.SetTag("Framework.Ticker.job.final_status", context.Status.ToString());
            jobActivity?.SetTag("Framework.Ticker.job.final_retry_count", context.RetryCount);
        ...
        else if (lastException != null)
        {
            ...
            jobActivity?.SetTag("Framework.Ticker.job.final_status", context.Status.ToString());
            jobActivity?.SetTag("Framework.Ticker.job.final_retry_count", context.RetryCount);
            jobActivity?.SetTag("Framework.Ticker.job.error_type", lastException.GetType().Name);
