...
/// <summary>
/// Builder for a single named captcha verifier inside <c>AddHeadlessCaptcha</c>. Provider packages contribute
/// exactly one provider per instance through <see cref="RegisterProvider"/> (called by each instance-scoped
/// <c>Use*</c> extension, for example <c>UseReCaptchaV2</c>, <c>UseReCaptchaV3</c>, <c>UseTurnstile</c>).
/// </summary>
[PublicAPI]
public sealed class HeadlessCaptchaInstanceBuilder
{
    ...
    /// <summary>The captcha verifier instance name. Used as the keyed-service key and the named-options name.</summary>
