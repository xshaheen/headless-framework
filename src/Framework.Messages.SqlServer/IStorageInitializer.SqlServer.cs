...
public class SqlServerStorageInitializer(
...
) : IStorageInitializer
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        var sql = CreateDbTablesScript(options.Value.Schema);
        var connection = new SqlConnection(options.Value.ConnectionString);
        await using var _ = connection.ConfigureAwait(false);
        object[] sqlParams =
        [
            new SqlParameter("@PubKey", $"publish_retry_{capOptions.Value.Version}"),
            new SqlParameter("@RecKey", $"received_retry_{capOptions.Value.Version}"),
