    ...
    where TCronTicker : CronTickerEntity, new()
{
    ...
    private async Task<InternalFunctionContext[]> _QueueNextTimeTickersAsync(
        TimeTickerEntity[] minTimeTickers,
        CancellationToken cancellationToken = default
    )
    {
        ...
        )
        {
            results.Add(
                new InternalFunctionContext
                {
                    ...
                    TimeTickerChildren = updatedTimeTicker
    ...
    )
    {
        ...
        var cronTickerIds = resources
            .Where(x => x.Type == TickerType.CronTickerOccurrence)
            .Select(x => x.TickerId)
            .ToArray();
        ...
        )
        {
            results.Add(
                new InternalFunctionContext
                {
                    ...
                    TimeTickerChildren = timedOutTimeTicker
                        .Children.Select(ch => new InternalFunctionContext
                        {
                            FunctionName = ch.Function,
                            TickerId = ch.Id,
