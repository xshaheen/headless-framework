...
const addHubListeners = async () => {
  props.jobNotificationHub.onReceiveUpdateCronJobOccurrence((val:GetCronJobOccurrenceResponse) => {
...
watch(
  ...
  () => {
    if (props.isOpen && props.dialogProps.id != undefined)
      props.jobNotificationHub.joinGroup(props.dialogProps.id)
    else if (!props.isOpen && props.dialogProps.id != undefined)
      props.jobNotificationHub.leaveGroup(props.dialogProps.id)
  }
)
