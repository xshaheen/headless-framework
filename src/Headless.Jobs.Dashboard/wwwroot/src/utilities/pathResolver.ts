/**
 * Utility functions for resolving paths using the Jobs configuration
 */


/**
 * Resolve a path relative to the Jobs base path
...
export function resolvePath(path: string): string {
  ...
  if (!config) {
    console.warn('Jobs configuration not found, using path as-is');
    return path;
  }
...
export function resolveApiUrl(endpoint: string): string {
  ...
  if (!config) {
    console.warn('Jobs configuration not found, using endpoint as-is');
    return endpoint;
  }
...
/**
 * Check if Jobs configuration is available
 * @returns True if configuration is available
 */
export function hasConfig(): boolean {
  return !!window.JobsConfig;
}
