...
| `ShutdownTimeout` | `TimeSpan` | `30s` | End-to-end messaging shutdown bound: broker and retry pickup close first, then locally accepted work drains with the remaining budget. Set the host/orchestrator termination grace above this value. Cleanup still running at the deadline remains fault-observed. `> 0`, `<= 5m`. |
