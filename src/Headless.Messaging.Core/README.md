...
Persisted retries use two independent timestamps: `NextRetryAt` controls when a row is due, and `LockedUntil` controls whether an active attempt still owns the row. Retry pickup filters on both. PostgreSQL and SQL Server compare and stamp them from one database-clock snapshot inside the atomic pickup command; in-memory storage uses its injected `TimeProvider`. Retry state writes clear `LockedUntil`; counter advances use an optimistic `Retries == originalRetries` predicate so concurrent replicas cannot overwrite each other's retry budget.
