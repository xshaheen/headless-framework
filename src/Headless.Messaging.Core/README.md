...
- callbacks are fire-and-forget async chaining, not request/reply. Set `PublishOptions.CallbackName` on the request and call `context.SetResponse<TResponse>(value)` inside the consumer to publish a typed response body through the durable bus path. No `SetResponse` keeps the callback headers-only; `SetResponse` without `CallbackName` is dropped. Callback delivery is at-least-once — make response consumers idempotent (dedupe on `(CorrelationId, CorrelationSequence)`; `CorrelationId` alone is ambiguous across hops because it is set to the immediate parent message id per hop, not the chain root).
