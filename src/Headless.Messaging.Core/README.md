...
Persisted retries use two independent timestamps: `NextRetryAt` controls when a row is due, and `LockedUntil` controls whether an active attempt still owns the row. Retry pickup filters on both. All providers compare `NextRetryAt` against the injected `TimeProvider` that created the schedule. PostgreSQL and SQL Server independently use one database-clock snapshot to compare lease expiry and stamp the next `LockedUntil` inside the same atomic pickup command; in-memory storage uses its injected `TimeProvider` for both responsibilities. Retry state writes clear `LockedUntil`; counter advances use an optimistic `Retries == originalRetries` predicate so concurrent replicas cannot overwrite each other's retry budget.
