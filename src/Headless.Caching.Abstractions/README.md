...
- `[PublicAPI] IRemoteTagMarkerCache` - optional capability a remote/L2 cache implements so a multi-tier host can push a tag/clear marker learned out-of-band (e.g. from a backplane invalidation notification) into the provider's process-local marker cache, avoiding the refresh-window wait. Providers that do not implement it simply fall back to window-bounded refresh.
