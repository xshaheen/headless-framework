...
- `[PublicAPI] IBufferCache` - capability interface for byte-oriented caches (Redis, InMemory, Hybrid) that read a payload into an `IBufferWriter<byte>` (`TryGetToAsync`) and write it from a `ReadOnlySequence<byte>` (`UpsertRawAsync`) without the intermediate `byte[]` the generic `GetAsync<byte[]>`/`UpsertEntryAsync<byte[]>` path allocates. The raw path stores the payload verbatim — it **bypasses the serializer** — so it is byte-cross-compatible with the typed path only on an instance whose serializer is the identity `RawBytesSerializer` (what the output-cache and BCL adapters wire); all expiry/tag/sliding/`CreatedAt` semantics match `UpsertEntryAsync`. The remaining copy per side is the unavoidable network I/O — zero-copy across a distributed cache is impossible.
