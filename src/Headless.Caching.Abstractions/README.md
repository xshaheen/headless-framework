...
- `[PublicAPI] ISeedableTagMarkerCache` - optional capability a cache implements so a multi-tier host can push a tag, clear, or remove-generation marker learned out-of-band (e.g. from a backplane invalidation notification) into the provider's process-local marker cache, avoiding the refresh-window wait. `SeedRemoveMarker` is the logical-`FlushAsync` counterpart: entries born before it read as a hard miss with no fail-safe reserve (unlike `SeedClearMarker`, which preserves reserves). Both `RedisCache` and `InMemoryCache` implement the interface — `InMemoryCache` treats `SeedRemoveMarker` as a no-op since its `FlushAsync` wipes physically. Providers that do not implement it fall back to window-bounded refresh.
