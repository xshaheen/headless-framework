...
public static class ReCaptchaResultExtensions
{
    ...
    public static ReCaptchaError[] ToReCaptchaErrors(this CaptchaVerifyResult result)
    {
        Argument.IsNotNull(result);

        return result.ErrorCodes is { } codes ? Array.ConvertAll(codes, static code => code.ToReCaptchaError()) : [];
    }
}
