...
{
    internal HeadlessFencingSetupBuilder(IServiceCollection services)
    {
    ...
    internal Action<FencingOptions>? OptionsConfigurator { get; private set; }

    internal IList<IFencingProviderOptionsExtension> Extensions { get; } = [];

    /// <summary>
    /// Configures <see cref="FencingOptions" />. Repeated calls run in call order against the same instance, so a
    /// later call overrides an earlier one.
    /// </summary>
    ...
    public HeadlessFencingSetupBuilder ConfigureOptions(Action<FencingOptions> configure)
    {
    ...
    /// <summary>Configures <see cref="FencingStorageOptions" />, the schema the lease table lives in.</summary>
    /// <param name="configure">Delegate that mutates the storage options.</param>
    /// <returns>This builder, to allow chaining.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="configure" /> is <see langword="null" />.</exception>
    public HeadlessFencingSetupBuilder ConfigureStorage(Action<FencingStorageOptions> configure)
    {
        Argument.IsNotNull(configure);
    ...
    /// <summary>
    /// Binds <see cref="FencingStorageOptions" /> from <paramref name="configuration" />; pass the
    /// <c>Headless:Fencing:Storage</c> section to bind <c>Headless:Fencing:Storage:Schema</c>.
    /// </summary>
    /// <param name="configuration">The configuration section to bind from.</param>
    /// <returns>This builder, to allow chaining.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="configuration" /> is <see langword="null" />.</exception>
    public HeadlessFencingSetupBuilder ConfigureStorage(IConfiguration configuration)
    {
        Argument.IsNotNull(configuration);

        Services.Configure<FencingStorageOptions>(configuration);
    ...
    public void RegisterExtension(IFencingProviderOptionsExtension extension)
    {
