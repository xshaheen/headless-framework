...
using Framework.Ticker.Utilities.DashboardDtos;
    ...
    where TCronTicker : CronTickerEntity, new()
{
    ...
    public async Task<IList<Tuple<TickerStatus, int>>> GetTimeTickerFullDataAsync(CancellationToken cancellationToken)
    {
        ...
        var result = allStatuses
            .Select(status => new Tuple<TickerStatus, int>(status, statusCounts.GetValueOrDefault(status, 0)))
            .ToList();
    ...
    )
    {
        ...
        var finalData = allDates
            .Select(date =>
            {
                ...
                return new TickerGraphData { Date = date, Results = results };
            })
            .ToList();
        return finalData;
    }

    public async Task<IList<TickerGraphData>> GetCronTickersGraphSpecificDataByIdAsync(
        Guid id,
        int pastDays,
        int futureDays,
        CancellationToken cancellationToken
    )
    {
        ...
        var cronTickerOccurrences = await _persistenceProvider.GetAllCronTickerOccurrences(
            (x => x.CronTickerId == id && x.ExecutionTime.Date >= startDate && x.ExecutionTime.Date <= endDate),
            cancellationToken
        );
    ...
    public async Task<IList<(int, int)>> GetLastWeekJobStatusesAsync(CancellationToken cancellationToken)
    {
        ...
        var timeTickerStatuses = timeTickers.Select(x => x.Status).ToList();
        ...
        var cronTickerStatuses = cronTickerOccurrences.Select(x => x.Status).ToList();
    ...
    )
    {
        return await _persistenceProvider.GetAllCronTickerOccurrences(
            x => x.CronTickerId == cronTickerId,
            cancellationToken
        );
    }

    public async Task<
        PaginationResult<CronTickerOccurrenceEntity<TCronTicker>>
    ...
    )
    {
        ...
        if (emptyPastSlots > 0)
        {
            ...
            {
                emptyPastDays.Add(new CronOccurrenceTickerGraphData { Date = firstPastDate.AddDays(-i), Results = [] });
            }
        }
        ...
        if (emptyFutureSlots > 0)
        {
            ...
            {
                emptyFutureDays.Add(
                    new CronOccurrenceTickerGraphData { Date = lastFutureDate.AddDays(i), Results = [] }
                );
            }
        }
