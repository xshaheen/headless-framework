...
public class Cache<TKey, T> : IDisposable
    where TKey : notnull
{
    ...
    private void _CheckTimer(TKey key, TimeSpan? cacheTimeout, bool restartTimerIfExists)
    {
        ...
        else
        {
            _timers.Add(
                key,
                new Timer(_RemoveByTimer, key, cacheTimeout ?? Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan)
            );
        }
    }

    private void _RemoveByTimer(object? state)
    {
        if (state is null)
        {
            return;
    ...
    /// <param name="key">The cache-key to add or update.</param>
    /// <param name="cacheObject">The cache object to store.</param>
    public void AddOrUpdate(TKey key, T? cacheObject)
    {
    ...
    public bool TryGet(TKey key, [NotNullWhen(true)] out T? value)
    {
        if (_disposed)
        {
            value = default;
...
public class MessagingCache : Cache<string, object>
{
    ...
    private static readonly Lazy<MessagingCache> _Global = new();
    ...
    /// </value>
    public static MessagingCache Global => _Global.Value;
