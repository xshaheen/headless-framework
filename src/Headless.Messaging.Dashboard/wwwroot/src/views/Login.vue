    ...
    <div class="login-card">
      <div class="login-header">
        <div class="logo-section">
          <img src="@/assets/logo.svg" alt="Messaging" class="logo" />
          <h1 class="app-title">Messaging Dashboard</h1>
        </div>
      ...
      >
        ...
        <div class="auth-help-text">
          ...
          Enter your dashboard credentials to access Messaging
        </div>
      </v-form>
      ...
      <div v-else-if="authMode === 'host'" class="host-auth-section">
        ...
        <v-form ref="form" class="login-form mt-4" @submit.prevent="handleLogin">
          <v-text-field
      ...
      <div v-else class="no-auth-message">
        ...
        <v-btn color="primary" @click="$router.push('/')"> Continue to Dashboard </v-btn>
      </div>
    </div>
  </div>
</template>
...
const isFormValid = computed(() => {
  if (authMode.value === 'basic') {
    return (
      (authStore.credentials.username?.length || 0) >= 3 &&
