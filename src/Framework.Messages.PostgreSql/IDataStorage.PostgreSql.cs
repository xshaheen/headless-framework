...
) : IDataStorage
{
    ...
    {
        await _ChangeMessageStateAsync(_pubName, message, state, transaction).ConfigureAwait(false);
    }

    public async Task ChangeReceiveStateAsync(MediumMessage message, StatusName state)
    {
        await _ChangeMessageStateAsync(_recName, message, state).ConfigureAwait(false);
    }
    ...
    public async Task StoreReceivedExceptionMessageAsync(string name, string group, string content)
    {
        ...
        await _StoreReceivedMessage(sqlParams).ConfigureAwait(false);
    }
    ...
    {
        return await _GetMessagesOfNeedRetryAsync(_pubName, lookbackSeconds).ConfigureAwait(false);
    }

    public async Task<IEnumerable<MediumMessage>> GetReceivedMessagesOfNeedRetry(TimeSpan lookbackSeconds)
    {
        return await _GetMessagesOfNeedRetryAsync(_recName, lookbackSeconds).ConfigureAwait(false);
    }
    ...
    private async Task _StoreReceivedMessage(object[] sqlParams)
    {
        var sql =
            ...
            + $"VALUES(@Id,'{capOptions.Value.Version}',@Name,@Group,@Content,@Retries,@Added,@ExpiresAt,@StatusName) RETURNING \"Id\";";
