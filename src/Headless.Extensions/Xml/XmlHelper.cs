...
public static class XmlHelper
{
    ...
    )
    {
        return str is not null
            ? await XmlEncodeAsIsAsync(str.RemoveHiddenChars(), settings, cancellationToken).ConfigureAwait(false)
            : null;
    }
    ...
    public static async Task<string> SerializeDateTimeAsync(
        DateTime dateTime,
        CancellationToken cancellationToken = default
    )
    {
    ...
    /// <returns><see langword="true"/> when the text parses as well-formed XML; otherwise <see langword="false"/>.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="maybeXml"/> is <see langword="null"/>.</exception>
    public static async Task<bool> IsValidXmlAsync(string maybeXml, CancellationToken cancellationToken = default)
    {
        using var xmlReader = XmlReader.Create(new StringReader(maybeXml), _SafeReaderSettings);

        return await _TryReadToEndAsync(xmlReader, cancellationToken).ConfigureAwait(false);
    }
    ...
    /// <returns><see langword="true"/> when the stream parses as well-formed XML; otherwise <see langword="false"/>.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="maybeXml"/> is <see langword="null"/>.</exception>
    public static async Task<bool> IsValidXmlAsync(Stream maybeXml, CancellationToken cancellationToken = default)
    {
        using var xmlReader = XmlReader.Create(maybeXml, _SafeReaderSettings);
...
#pragma warning restore MA0045
    ...
    private static async Task<bool> _TryReadToEndAsync(XmlReader xmlReader, CancellationToken cancellationToken)
    {
        try
        {
            while (await xmlReader.ReadAsync().ConfigureAwait(false))
            {
                // XmlReader.ReadAsync has no cancellation overload; observe the token between reads instead.
                cancellationToken.ThrowIfCancellationRequested();
            }
