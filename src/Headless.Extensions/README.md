...
- **Result/error types preserve structured API errors and reject invalid default state.** `Conflict(...)` accepts one or many `ErrorDescriptor` values; authorization and validation factories preserve descriptor codes and parameters; collections are snapshotted. `ApiResultError.Code` / `Metadata` remain free of equality-corrupting lazy caches. A default `ApiResult` / `ApiResult<T>` is uninitialized (neither success nor failure); try-accessors return false and branch operations throw.
...
- **`ObjectPropertiesHelper.TrySetPropertyValue` evaluates its argument eagerly.** The value-direct overload exists so hot paths can skip the per-call lambda of `TrySetProperty(obj, name, () => value)`, but the argument is computed whether or not the target property exists. Keep clock reads, GUID minting, and other observable-on-evaluation values on the `Func` overload (its factory runs only when the property is actually set); pass only already-materialized values directly. The `TValue`-vs-`Func<TValue>` pair is named differently on purpose: a `TValue` overload beside `Func<TValue>` is ambiguous for `Func`-typed arguments.
