...
public sealed class SqliteResourceLockStorage(SqliteConnection connection) : IResourceLockStorage
{
    ...
    private const string _InsertSql = """
        DELETE FROM ResourceLocks WHERE exp < @now;
        INSERT INTO ResourceLocks (res, lockId, exp) VALUES (@res, @lockId, @exp)
        """;

    private const string _ReplaceIfEqualSql = """
        UPDATE ResourceLocks
        SET lockId = @lockId, exp = @exp
    ...
    private const string _GetExpirationSql = "SELECT exp FROM ResourceLocks WHERE res = @res";
    private const string _IsExistSql = "SELECT COUNT(1) FROM ResourceLocks WHERE res = @res";
    ...
    public async ValueTask<bool> InsertAsync(string key, string lockId, TimeSpan? expiration = null)
    {
        ...
        command.Parameters.Add(new SqliteParameter("@exp", _GetTimeSpanParameterValue(expiration)));
