...
/// deliberate contract for connection-scoped locks (mirroring <see langword="lock"/>/<see langword="using"/> discipline); the reference
...
/// </summary>
/// <remarks>Initializes a new handle for an acquired connection-scoped lock.</remarks>
...
) : IDistributedLease
{
    ...
    /// </summary>
    public CancellationToken LostToken => handle.ConnectionLostToken;
    ...
    /// </summary>
    public bool CanObserveLoss => handle.ConnectionLostToken.CanBeCanceled;
    ...
    public async Task ReleaseAsync()
    {
        ...
        using (await _gate.LockAsync(CancellationToken.None).ConfigureAwait(false))
        {
            ...
            try
            {
                await release(handle, CancellationToken.None).ConfigureAwait(false);
