...
internal static class HybridCacheInvalidationConsumerRegistration
{
    ...
    /// framework's internal immutable Bus contribution, which messaging bootstrap drains into the consumer registry.
    /// One consumer serves every hybrid (default and named): it routes each incoming
    ...
    /// application's own explicit Bus <c>ForMessage&lt;CacheInvalidationMessage&gt;(…)</c> — subsequent calls are
    ...
    /// </para>
    /// <para>
    /// <b>Unconditional and order-independent.</b> The consumer contribution is registered regardless
    /// of whether a bus is present yet: the emitted descriptors are inert until messaging bootstrap drains them
    /// from the built provider, and the contribution can be added before or after
    ...
    {
        // Idempotent: an application's own Bus registration, or a prior call here for another
