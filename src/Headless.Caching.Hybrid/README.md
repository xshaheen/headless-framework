...
Initial and replayed invalidations use `DeliveryMode.Direct`, so an active unit of work never captures or delays the backplane signal — `Direct` always bypasses storage and enlistment. `HybridCache` is a framework-internal singleton; its DI factory builds this bus directly over the messaging core (`new Bus(sp.GetRequiredService<MessagePublisher>())`), never resolving the scoped `IBus`, so it needs no service scope of its own. Publish failures are non-fatal. Other instances may keep their L1 value until TTL or the next successful invalidation, while the local instance still observes the write result.
...
cache.Events.Invalidation.AddHandler(e => logger.LogDebug("invalidation {Kind} {Direction}", e.Kind, e.Direction));
```
