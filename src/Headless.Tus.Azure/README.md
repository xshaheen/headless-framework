...
**Cancellation policy (TusDiskStore parity).** Client reads use the request token — it is the only disconnect signal — while everything that persists already-received bytes runs on `CancellationToken.None`: block staging, block-list commits, chunk-tracking metadata, sliding-expiration refresh, and checksum verification/rollback. A disconnect therefore commits the bytes received so far (the tus spec: the server "SHOULD always attempt to store as much of the received data as possible") and the client resumes from them instead of re-uploading. One exception: the `EnableChunkSplitting=false` fast path for seekable bodies streams directly into `StageBlock` with the live token — tusdotnet request bodies are never seekable, so that path is store-direct only.
