...
- During composite formation, finite-TTL children are renewed at half the TTL, capped at one minute, unless `LockMonitoringMode.AutoExtend` already owns renewal. A semaphore slot always has a finite TTL, so formation renewal always applies there. The cadence follows the TTL the backend really applies rather than the one requested: a reader-writer **read** child cannot hold an infinite lease (its reader entry needs a finite expiry, or a crashed reader strands the resource), so the provider clamps `Timeout.InfiniteTimeSpan` to `DefaultTimeUntilExpires` and a set containing any `Read` entry is renewed during formation even when an infinite TTL was requested. Renewal forwards the requested TTL, so each child re-applies its own clamp and a mixed set's write child keeps its infinite lease. Composite deadlines and waits use the provider's `TimeProvider`; custom providers must expose the clock used by their own acquisition logic. That clock schedules the check-in; it never arbitrates expiry — only the backend's own answer decides whether a lease still holds.
