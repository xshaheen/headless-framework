...
/// <summary>
/// Adds the enlisted transaction-lock accessor to <see cref="IUnitOfWork" />, so a lock that must die with the
/// unit's transaction is reached from the unit it joins and needs no provider transaction type to discover.
/// </summary>
[PublicAPI]
public static class HeadlessUnitOfWorkTransactionLocksExtensions
{
    private const string _NotRegisteredMessage =
        "No transaction-scoped lock provider is registered for this unit of work. Configure "
    ...
    {
        /// <summary>
        /// Gets the transaction-scoped locks bound to this handle: a lock taken through them lives inside this
        /// unit's transaction, and the unit's commit or rollback releases it.
        /// </summary>
        /// <remarks>
        ...
        /// No transaction-scoped lock provider is registered in this host, or the unit is no longer active.
        /// </exception>
        /// <exception cref="ObjectDisposedException">This handle was disposed.</exception>
        public UnitOfWorkTransactionLocks TransactionLocks
        {
            get
            {
                ...
                var feature =
                    unitOfWork.GetFeature<IUnitOfWorkTransactionLocks>()
                    ?? throw new InvalidOperationException(_NotRegisteredMessage);

                return unitOfWork.GetOrAdd(
                    feature,
                    static (unit, locks) => new UnitOfWorkTransactionLocks(locks, unit)
                );
            }
        }
    }
