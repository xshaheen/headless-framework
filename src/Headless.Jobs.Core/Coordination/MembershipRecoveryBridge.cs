...
/// <summary>
/// Drives dead-node reclaim from the membership substrate: reclaims on <see cref="NodeLeft"/> events and
/// runs a periodic liveness-snapshot reconcile to catch any death missed while not subscribed.
/// </summary>
/// <remarks>
/// Membership events are best-effort acceleration (origin §4b); the periodic reconcile is the authoritative
/// backstop. Reclaim is idempotent — an in-memory reclaimed-set suppresses duplicate work between the event
/// and reconcile paths, and the strict conditional reclaim UPDATE makes a repeated reclaim a no-op. Reclaim
/// writes use <see cref="CancellationToken.None"/> (KTD6) so a reclaim racing host shutdown is not torn
/// mid-write. A failed reclaim is logged and removed from the set so the next reconcile tick retries it.
/// </remarks>
internal sealed class MembershipRecoveryBridge(
    INodeMembership membership,
    IInternalJobManager internalJobManager,
    SchedulerOptionsBuilder optionsBuilder,
    TimeProvider timeProvider,
    ILogger<MembershipRecoveryBridge> logger
) : BackgroundService
{
    ...
    private async Task _ReconcileLoopAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                await timeProvider.Delay(optionsBuilder.DeadNodeReconcileInterval, stoppingToken).ConfigureAwait(false);
...
internal static partial class MembershipRecoveryBridgeLog
{
    [LoggerMessage(
        EventId = 1,
        EventName = "MembershipWatchFailed",
        Level = LogLevel.Error,
        Message = "Membership watch loop failed; dead-node recovery falls back to the periodic reconcile"
    )]
    ...
    [LoggerMessage(
        ...
        Level = LogLevel.Error,
        Message = "Dead-node liveness reconcile failed"
    )]
    ...
    [LoggerMessage(
        ...
        Level = LogLevel.Error,
        Message = "Failed to reclaim resources for dead node {Owner}; will retry on the next reconcile"
    )]
