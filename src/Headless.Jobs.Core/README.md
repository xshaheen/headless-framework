...
**Cron-seed migration** (`jobs.cron-seed-migration`) runs on every node at boot — each scans code-defined cron jobs and upserts them, so a rolling deploy of N nodes is N full scans + N write storms. `MigrateDefinedCronJobs` is a *find-and-update*: **sequential** re-runs are idempotent (a second run updates the row in place and touches zero new rows). It is **not** unique-constrained on `Function`, though, so **simultaneous** first-boot across nodes can each insert a distinct duplicate seed row, double-scheduling that function. The optional Jobs-scoped distributed lock removes the redundant cross-node scan/write storm **and** suppresses that concurrent-first-boot duplicate-seed race — best-effort (the lock is single-instance, not consensus-grade). It is never the boundary for job-row *ownership/execution* (per-row predicates, `node@incarnation` ownership, and per-job leases stay that boundary). A unique index on the seeded `Function` would make duplicate suppression a hard guarantee (tracked as a follow-up).
