...
`JobOptions` and `RecurringJobOptions` expose description, durable retries/intervals, node-death policy, and `RequireAtomicEnlistment`; recurring options also expose nullable IANA `TimeZoneId`. Execution time and cron expression are explicit method arguments. Priority remains immutable `[JobFunction]` / descriptor metadata.
...
Policies apply to facade one-shot scheduling, keyed scheduling/replacement, every chain node, facade recurring definitions, and required atomic assertions on keyed cancellation using its scope function. Facade recurring definitions inherit retries, node-death policy, and a function-level atomic requirement (never the host default); `RecurringJobOptions.RequireAtomicEnlistment` asserts the same requirement per call. Attribute-seeded cron definitions and low-level manager calls retain their own settings; seeding is exempt from atomic policy because it runs before any application transaction exists. Ordinary ID-only cancellation and cron pause/resume retain their existing control semantics.
