...
Beyond metrics and traces, `cache.Events` (`ICacheEvents`) is a second, in-process consumer channel over the same signals — an `IAsyncEvent<TArgs>` per signal (`Hit`/`Miss`/`Set`/`Remove`, factory/fail-safe/refresh, in-memory `Eviction`, hybrid `Invalidation`) dispatched by `CacheEventsHub`. Handlers (async or sync) run guarded on a background task by default; set `AddHeadlessCaching(s => s.SyncHandlers = true)` to run synchronous handlers inline on the firing thread and `s.EventHandlerErrorLogLevel` (default `Warning`) to set the handler-error log level. An event with no handler builds no args and does no work. See [docs/llms/caching.md](../../docs/llms/caching.md) for the full event list and execution semantics.
