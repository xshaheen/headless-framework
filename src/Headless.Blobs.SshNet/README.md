...
- **A missing destination container fails loudly, never as "source not found".** `CopyAsync`/`MoveAsync` return `false` only when the *source* blob is absent. Copying into a top-level container that was never provisioned throws `DirectoryNotFoundException` — the same type and data-plane rule as the FileSystem provider, so provider-portable code catches one exception for both. (AWS/Azure cannot make this distinction: their wire 404 covers a missing source and a missing destination container alike, so both collapse to `false` there.)
