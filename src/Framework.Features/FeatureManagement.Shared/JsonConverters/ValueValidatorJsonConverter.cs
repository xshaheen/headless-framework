    ...
    public override IValueValidator Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        if (nameJsonProperty.Value.ValueKind == JsonValueKind.String)
        {
            ...
            var propertiesJsonProperty = rootElement
                .EnumerateObject()
                .FirstOrDefault(x =>
                    x.Name.Equals(nameof(IValueValidator.Properties), StringComparison.OrdinalIgnoreCase)
                );
            if (propertiesJsonProperty.Value.ValueKind == JsonValueKind.Object)
            {
                ...
                var properties = propertiesJsonProperty.Value.Deserialize<IDictionary<string, object>>(
