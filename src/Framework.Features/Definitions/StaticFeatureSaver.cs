...
public class StaticFeatureSaver : IStaticFeatureSaver
{
    ...
    public async Task SaveAsync()
    {
        ...
        )
        {
            ...
            {
                /* It will re-try */
                throw new InvalidOperationException("Could not acquire distributed lock for saving static features!");
            }
    ...
    )
    {
        ...
        var featureGroupRecordsInDatabase = (await FeatureGroupRepository.GetListAsync()).ToDictionary(x => x.Name);
    ...
    private async Task<bool> UpdateChangedFeaturesAsync(IEnumerable<FeatureDefinitionRecord> featureRecords)
    {
        var newRecords = new List<FeatureDefinitionRecord>();
        var changedRecords = new List<FeatureDefinitionRecord>();

        var featureRecordsInDatabase = (await FeatureRepository.GetListAsync()).ToDictionary(x => x.Name);
        ...
        if (FeatureOptions.DeletedFeatures.Any())
        {
            deletedRecords.AddRange(
                featureRecordsInDatabase.Values.Where(x => FeatureOptions.DeletedFeatures.Contains(x.Name))
            );
        }
    ...
    )
    {
        var jsonSerializerOptions = new JsonSerializerOptions
        {
            TypeInfoResolver = new DefaultJsonTypeInfoResolver
            {
                Modifiers =
                {
                    new AbpIgnorePropertiesModifiers<FeatureGroupDefinitionRecord, Guid>().CreateModifyAction(x =>
                        x.Id
                    ),
