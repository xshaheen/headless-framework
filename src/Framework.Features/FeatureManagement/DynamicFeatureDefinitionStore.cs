...
public sealed class DynamicFeatureDefinitionStore : IDynamicFeatureDefinitionStore
{
    ...
    private async Task _EnsureCacheIsUptoDateAsync()
    {
        ...
        if (stampInDistributedCache == _storeCache.CacheStamp)
        {
    ...
    {
        var featureGroupRecords = await _featureGroupRepository.GetListAsync();
        var featureRecords = await _featureRepository.GetListAsync();
    ...
    private async Task<string> _GetOrSetStampInDistributedCache()
    {
        ...
        await using (
            var commonLockHandle = await _distributedLock.TryAcquireAsync(
        ...
        )
        {
            if (commonLockHandle == null)
            {
                ...
                throw new AbpException("Could not acquire distributed lock for feature definition common stamp check!");
            }
