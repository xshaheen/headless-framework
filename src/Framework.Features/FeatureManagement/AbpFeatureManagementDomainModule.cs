...
/*
[DependsOn(typeof(AbpFeatureManagementDomainSharedModule), typeof(AbpFeaturesModule), typeof(AbpCachingModule))]
public class AbpFeatureManagementDomainModule : AbpModule
{
    ...
    {
        var options = context.ServiceProvider.GetRequiredService<IOptions<FeatureManagementOptions>>().Value;
            ...
            .ExecuteAsync(
                async _ =>
                {
                    try
                    {
                        ...
                        await scope.ServiceProvider.GetRequiredService<IStaticFeatureSaver>().SaveAsync();
                    }
                    catch (Exception ex)
                    {
                        // ReSharper disable once AccessToDisposedClosure
                        scope
                            .ServiceProvider.GetService<ILogger<AbpFeatureManagementDomainModule>>()
                            ?.LogException(ex);
    ...
    private static async Task PreCacheDynamicFeaturesAsync(FeatureManagementOptions options, IServiceScope scope)
    {
        ...
        try
        {
            ...
            await scope.ServiceProvider.GetRequiredService<IDynamicFeatureDefinitionStore>().GetGroupsAsync();
        }
        catch (Exception ex)
        {
            // ReSharper disable once AccessToDisposedClosure
            scope.ServiceProvider.GetService<ILogger<AbpFeatureManagementDomainModule>>()?.LogException(ex);
