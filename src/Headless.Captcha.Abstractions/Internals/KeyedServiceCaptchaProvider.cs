...
/// </summary>
internal sealed class KeyedServiceCaptchaProvider(IServiceProvider serviceProvider) : ICaptchaProvider
{
    public ICaptchaVerifier GetVerifier(string name)
    {
        ...
        return serviceProvider.GetKeyedService<ICaptchaVerifier>(name)
            ?? throw new InvalidOperationException(
                $"No captcha verifier is registered under the name '{name}'. Register it through the builder first — "
                    + $"for example AddHeadlessCaptcha(b => b.UseTurnstile(\"{name}\", …)) or b.UseReCaptchaV3(\"{name}\", …) — "
                    + "or use a default provider's canonical key (one of the 'Headless.Captcha:' constants on CaptchaConstants)."
            );
    }
