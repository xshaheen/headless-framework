...
/// arrives without a completion verb after a completed transaction is logged by the manager as forgotten.
/// </summary>
...
public sealed class PostgreSqlUnitOfWorkTests(PostgreSqlUnitOfWorkFixture fixture) : TestBase
{
    ...
    public async Task should_warn_and_discard_when_an_observed_unit_is_disposed_without_a_completion_verb_after_commit()
    {
        ...
        await using (var unitOfWork = manager.Enlist(connection, transaction))
        {
            ...
            await _TouchAsync(connection, transaction);
            await transaction.CommitAsync(AbortToken);
            // Forgotten completion: Npgsql exposes no commit edge to observe for the caller.
        }
    ...
    private static NpgsqlTransaction _Transaction(IUnitOfWork unitOfWork)
    {
        return (NpgsqlTransaction)((IRelationalUnitOfWorkResource)unitOfWork.Resource!).Transaction;
    }
