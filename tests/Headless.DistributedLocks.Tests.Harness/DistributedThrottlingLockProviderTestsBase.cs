...
public abstract class DistributedThrottlingLockProviderTestsBase : TestBase
{
    protected TimeProvider TimeProvider { get; } = TimeProvider.System;

    protected abstract IThrottlingDistributedLockStorage GetLockStorage();

    protected IThrottlingDistributedLockProvider GetLockProvider(int maxHits, TimeSpan period)
    {
        ...
        return new ThrottlingDistributedLockProvider(
            GetLockStorage(),
            options,
            TimeProvider,
            LoggerFactory.CreateLogger<ThrottlingDistributedLockProvider>()
        );
    }
    ...
    private async Task _AcquireLocksSync(IThrottlingDistributedLockProvider provider, string resource, int count)
    {
    ...
    private async Task _AcquireLocksConcurrently(
        IThrottlingDistributedLockProvider provider,
        string resource,
        int count
    )
    {
    ...
    private async Task _AssertCannotAcquireMore(IThrottlingDistributedLockProvider provider, string resource)
    {
        var timestamp = Stopwatch.GetTimestamp();
