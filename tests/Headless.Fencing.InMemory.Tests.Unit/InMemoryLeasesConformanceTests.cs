...
// Omitted scenarios, each for the reason given:
// - should_decide_expiry_by_the_database_clock_whatever_the_application_clock: the injected TimeProvider is this
//   provider's clock authority by design, so there is no separate application clock to ignore.
//   InMemoryLeaseStoreTests drives expiry with FakeTimeProvider instead.
// - should_mark_only_an_observed_unit_non_retryable_and_only_for_writes: an observed unit wraps a caller-owned
//   database transaction, which this provider refuses.
// - should_refuse_a_unit_on_another_database_before_any_statement: there is no database to compare.
//   InMemoryLeaseStoreTests covers the refusal of any unit over a database connection.
[Collection<InMemoryFencingFixture>]
public sealed class InMemoryLeasesConformanceTests(InMemoryFencingFixture fixture)
    : LeasesConformanceTests<InMemoryFencingFixture>(fixture)
{
    [Fact]
