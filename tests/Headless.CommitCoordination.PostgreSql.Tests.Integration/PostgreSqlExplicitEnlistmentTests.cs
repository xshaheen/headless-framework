...
/// <summary>
/// The explicit-signal contract of <c>NpgsqlConnection.EnlistCommitCoordination</c> against a real PostgreSQL:
/// the caller signals after the transaction completes, and a dispose that arrives un-signalled after a completed
/// transaction is logged as a forgotten signal.
/// </summary>
[Collection<PostgreSqlCoordinatedTransactionFixture>]
public sealed class PostgreSqlExplicitEnlistmentTests(PostgreSqlCoordinatedTransactionFixture fixture) : TestBase
{
    private const string _ScopeCategory = "Headless.CommitCoordination.PostgreSql.PostgreSqlCommitScope";
    ...
    public async Task should_warn_and_discard_when_disposed_without_a_signal_after_commit()
    {
        ...
        await using (var scope = connection.EnlistCommitCoordination(transaction, services))
        {
            ...
            await _TouchAsync(connection, transaction);
            await transaction.CommitAsync(AbortToken);
            // Forgotten signal: Npgsql exposes no commit edge to observe for the caller.
        }
    ...
    private static ServiceProvider _BuildServices(CapturingLoggerProvider logs)
    {
        return new ServiceCollection()
            ...
            .AddPostgreSqlCommitCoordination()
            .BuildServiceProvider();
    }

    private static async Task _TouchAsync(NpgsqlConnection connection, NpgsqlTransaction transaction)
    {
