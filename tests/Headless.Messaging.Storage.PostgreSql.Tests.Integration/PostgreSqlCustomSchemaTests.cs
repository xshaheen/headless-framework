...
/// <summary>
/// Proves the feature-owned schema reaches every PostgreSQL storage path: the initializer creates the
...
public sealed class PostgreSqlCustomSchemaTests(PostgreSqlTestFixture fixture) : TestBase
{
    ...
    public async Task should_create_and_use_the_tables_in_the_configured_schema()
    {
        ...
        finally
        {
            ...
            await cleanup.ExecuteAsync($"""DROP SCHEMA IF EXISTS "{_Schema}" CASCADE;""");
        }
    }
}
