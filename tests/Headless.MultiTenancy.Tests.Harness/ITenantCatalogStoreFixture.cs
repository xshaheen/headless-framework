...
/// starts, via its fail-fast-at-startup contract); the EF store inserts <c>TenantRecord</c> rows
/// directly against the container database (duplicates fail on the unique-index violation).
...
/// <see cref="ITenantDirectory"/> — all v1 stores enumerate.
/// </summary>
public interface ITenantCatalogStoreFixture
{
    ...
    /// </summary>
    /// <param name="seeds">Already-normalized tenant seed data — the store must not re-normalize.</param>
    /// <param name="cancellationToken">A token to cancel the operation.</param>
    /// <returns>An <see cref="ITenantStore"/> that also implements <see cref="ITenantDirectory"/>.</returns>
    /// <exception cref="Exception">
    /// Two or more seeds normalize to the same identifier. The concrete exception type is
