...
/// <summary>One captured log record with its category, as recorded by <see cref="CapturingLoggerProvider" />.</summary>
public sealed record CategoryLogEntry(string Category, LogLevel Level, EventId EventId, string Message);

/// <summary>
/// An <see cref="ILoggerProvider" /> that records every entry from every category, so a scenario can assert what
/// a DI-resolved component (a provider enlistment scope, the interceptor) logged — or that it logged nothing.
/// </summary>
