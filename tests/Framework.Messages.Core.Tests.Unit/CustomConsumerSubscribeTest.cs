...
{
    private const string _TopicNamePrefix = "topic";
    private const string _GroupNamePrefix = "group";
    ...
    public CustomConsumerSubscribeTest()
    {
        ...
        services.AddCap(x =>
        {
            x.TopicNamePrefix = _TopicNamePrefix;
            x.GroupNamePrefix = _GroupNamePrefix;
        });
    ...
    public void CanFindSpecifiedTopic()
    {
        var selector = _provider.GetRequiredService<IConsumerServiceSelector>();
        var candidates = selector.SelectCandidates();
        var bestCandidates = selector.SelectBestCandidate($"{_TopicNamePrefix}.Candidates.Foo", candidates);

        Assert.NotNull(bestCandidates);
        Assert.NotNull(bestCandidates.MethodInfo);
        Assert.StartsWith(_GroupNamePrefix, bestCandidates.Attribute.Group);
        Assert.StartsWith(_TopicNamePrefix, bestCandidates.TopicName);
        Assert.Equal(typeof(Task), bestCandidates.MethodInfo.ReturnType);
    }
}

public class MyConsumerServiceSelector(IServiceProvider serviceProvider) : ConsumerServiceSelector(serviceProvider)
{
    private readonly CapOptions _capOptions = serviceProvider.GetRequiredService<IOptions<CapOptions>>().Value;
    ...
    )
    {
        ...
        using (var scoped = provider.CreateScope())
        {
            ...
            foreach (var service in consumerServices)
            {
                ...
                executorDescriptorList.AddRange(_GetMyDescription(typeInfo));
            }
    ...
    private IEnumerable<ConsumerExecutorDescriptor> _GetMyDescription(TypeInfo typeInfo)
    {
        foreach (var method in typeInfo.DeclaredMethods)
        {
            ...
            foreach (var attr in topicAttributes)
            {
                ...
                {
                    Attribute = new CapSubscribeAttribute(attr.Name) { Group = attr.Group },
...
public class MySubscribeAttribute(string name) : Attribute
{
