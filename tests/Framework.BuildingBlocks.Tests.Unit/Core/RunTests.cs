...
public sealed class RunTests : TestBase
{
    ...
    public async Task should_throw_after_max_attempts_exceeded()
    {
        ...
        // when
        var action = async () => await Run.WithRetriesAsync(callback, maxAttempts: maxAttempts, cancellationToken: AbortToken);
    ...
    public async Task should_use_custom_retry_interval()
    {
        ...
        // when
        var task = Run.WithRetriesAsync(callback, maxAttempts: 5, retryInterval: retryInterval, timeProvider: fakeTime, cancellationToken: AbortToken);
