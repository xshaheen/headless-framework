...
/// <summary>
/// Regression guard for the captive-dependency fix: the EF grant repository is a singleton but must not
...
{
    [Fact]
    public void ef_grant_repository_should_resolve_as_singleton_when_scoped_event_bus_is_registered()
    {
        // given - a host that wires the scoped ILocalEventBus alongside EF permissions storage, with scope validation on
        var services = new ServiceCollection();
        services.AddDbContextFactory<ScopeTestDbContext>(o => o.UseSqlite("DataSource=:memory:"));
        services.AddHeadlessLocalEventBus();
        services.AddHeadlessPermissions(setup => setup.UseEntityFramework<ScopeTestDbContext>());

        using var provider = services.BuildServiceProvider(validateScopes: true);

        // when - the singleton repo is resolved from the root scope
        var act = () => provider.GetRequiredService<IPermissionGrantRepository>();

        // then - no captive-dependency violation and the repo stays a singleton
        var repo = act.Should().NotThrow().Which;
        provider.GetRequiredService<IPermissionGrantRepository>().Should().BeSameAs(repo);
    }
