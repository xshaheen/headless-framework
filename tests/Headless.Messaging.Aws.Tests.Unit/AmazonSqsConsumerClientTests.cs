...
public sealed class AmazonSqsConsumerClientTests : TestBase
{
    ...
    public async Task should_invoke_callback_with_correct_message_headers()
    {
        ...
        capturedSender.Should().BeOfType<InflightSqsMessage>();
        var inflight = (InflightSqsMessage)capturedSender!;
        inflight.QueueUrl.Should().Be("http://test");
        inflight.ReceiptHandle.Should().Be("receipt-header-test");
    }
