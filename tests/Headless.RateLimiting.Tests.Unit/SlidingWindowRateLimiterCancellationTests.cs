...
public sealed class SlidingWindowDistributedRateLimiterCancellationTests : TestBase
{
    private readonly FakeTimeProvider _timeProvider = new();
    private readonly ILogger<SlidingWindowDistributedRateLimiter> _logger = Substitute.For<
        ILogger<SlidingWindowDistributedRateLimiter>
    >();
    ...
    {
        // given
        var storage = Substitute.For<IDistributedRateLimiterStorage>();
        var options = new SlidingWindowRateLimiterOptions { MaxHitsPerPeriod = 1 };
        var sut = new SlidingWindowDistributedRateLimiter(storage, options, _timeProvider, _logger);
