...
public sealed class ResourceLockTusFileLockTests : TestBase
{
    ...
    public async Task should_acquire_lock_via_provider()
    {
        ...
        var sut = new ResourceLockTusFileLock(fileId, _resourceLockProvider);
    ...
    public async Task should_release_lock_when_held()
    {
        ...
        var resourceLock = Substitute.For<IResourceLock>();
        resourceLock.Resource.Returns("tus-file-lock-test-file");
        resourceLock.LockId.Returns(lockId);
    ...
    public async Task should_noop_release_when_not_held()
    {
        ...
        var sut = new ResourceLockTusFileLock(fileId, _resourceLockProvider);
        await sut.Lock(); // Lock fails, _resourceLock remains null
