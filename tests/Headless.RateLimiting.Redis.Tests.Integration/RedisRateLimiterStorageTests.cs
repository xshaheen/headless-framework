...
/// <summary>
/// Integration tests for <see cref="RedisDistributedRateLimiterStorage"/>.
/// Tests verify Redis-specific behaviors: INCRBY with expiration via Lua script.
/// </summary>
[Collection<RedisTestFixture>]
public sealed class RedisDistributedRateLimiterStorageTests(RedisTestFixture fixture) : TestBase
{
    ...
    public async Task should_increment_and_return_new_value()
    {
        ...
        // when
        var result1 = await fixture.RateLimiterStorage.IncrementAsync(resource, ttl);
        var result2 = await fixture.RateLimiterStorage.IncrementAsync(resource, ttl);
        var result3 = await fixture.RateLimiterStorage.IncrementAsync(resource, ttl);
    ...
    public async Task should_set_expiration_on_first_increment()
    {
        ...
        // when
        await fixture.RateLimiterStorage.IncrementAsync(resource, ttl);
    ...
    public async Task should_expire_counter_after_ttl()
    {
        ...
        // then
        var count = await fixture.RateLimiterStorage.GetHitCountsAsync(resource);
        count.Should().Be(0);
    }
    ...
    public async Task should_increment_atomically_under_concurrent_access()
    {
        ...
        await Parallel.ForEachAsync(
            ...
            async (_, _) => await fixture.RateLimiterStorage.IncrementAsync(resource, ttl, AbortToken)
        );
