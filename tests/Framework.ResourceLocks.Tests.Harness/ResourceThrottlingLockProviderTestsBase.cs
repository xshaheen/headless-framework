...
public abstract class ResourceThrottlingLockProviderTestsBase(ITestOutputHelper output) : TestBase(output)
{
    ...
    public virtual async Task should_throttle_calls_async()
    {
        ...
        await _AssertCanAcquireWithGreaterWait(lockProvider, resource);
    }
    ...
    private async Task _AssertCanAcquireWithGreaterWait(IResourceThrottlingLockProvider provider, string resource)
    {
        var timestamp = Stopwatch.GetTimestamp();
        var result = await provider.TryAcquireAsync(resource, acquireTimeout: TimeSpan.FromSeconds(2.5));
        var elapsed = Stopwatch.GetElapsedTime(timestamp);
