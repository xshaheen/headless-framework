...
public sealed class DeliveryEnumArityTests : TestBase
{
    ...
    public void should_pin_delivery_mode_members_to_the_resolver_guard()
    {
        Enum.GetValues<DeliveryMode>()
            .Should()
            .Equal(
                [DeliveryMode.Durable, DeliveryMode.Coordinated, DeliveryMode.Direct],
                "DeliveryDecisionResolver.Resolve rejects anything outside `requestedMode is (Durable or Coordinated or Direct)`, and its resolvedMode/path switches would hit UnreachableException for a new member"
            );
    }
