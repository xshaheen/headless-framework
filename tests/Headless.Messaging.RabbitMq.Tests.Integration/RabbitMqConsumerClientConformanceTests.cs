    ...
    : TransportConsumerConformanceTestsBase
{
    ...
    [Fact]
    public async Task should_drain_legacy_exchange_before_lane_cutover_and_reconcile_forward()
    {
        var exchangeName = $"migration-{Guid.NewGuid():N}";
        var legacyQueue = $"legacy-{Guid.NewGuid():N}";
        var logicalName = $"orders-{Guid.NewGuid():N}";
        ...
        await channel.QueueBindAsync(legacyQueue, exchangeName, logicalName, cancellationToken: AbortToken);
        ...
        var legacyDelivery = await channel.BasicGetAsync(legacyQueue, autoAck: false, AbortToken);
        legacyDelivery.Should().NotBeNull("the deployment fence must observe and drain legacy backlog");
        await channel.BasicAckAsync(legacyDelivery!.DeliveryTag, multiple: false, AbortToken);
        (await channel.BasicGetAsync(legacyQueue, autoAck: false, AbortToken)).Should().BeNull();
        ...
        (await channel.BasicGetAsync(legacyQueue, autoAck: false, AbortToken))
            .Should()
            .BeNull("post-cutover publication is roll-forward-only and must not return to the legacy exchange");
    }
