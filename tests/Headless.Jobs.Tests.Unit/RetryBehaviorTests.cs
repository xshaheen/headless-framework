...
public sealed class RetryBehaviorTests : TestBase
{
    ...
    [Fact]
    public async Task ExecuteTaskAsync_observes_pre_registration_durable_cancellation_before_user_code()
    {
    ...
    [Fact]
    public async Task ExecuteTaskAsync_keeps_host_shutdown_distinct_from_durable_cancellation()
    {
        var (handler, context, manager, _) = _SetupRetryTestFixture([0], retries: 0);
        context.Type = JobType.TimeJob;
    ...
    [Fact]
    public async Task ExecuteTaskAsync_observes_durable_cancellation_on_the_configured_interval()
    {
    ...
    [Fact]
    public async Task ExecuteTaskAsync_preserves_an_uncooperative_handlers_natural_result()
    {
        var timeProvider = new FakeTimeProvider();
        var schedulerOptions = new SchedulerOptionsBuilder
        {
    ...
    [Fact]
    public async Task ExecuteTaskAsync_preserves_an_uncooperative_handlers_natural_failure()
    {
        var timeProvider = new FakeTimeProvider();
        var schedulerOptions = new SchedulerOptionsBuilder
        {
    ...
    [Fact]
    public async Task ExecuteTaskAsync_aborts_on_initial_observation_failure_and_allows_later_recovery_execution()
    {
        var (handler, context, manager, _) = _SetupRetryTestFixture([0], retries: 0);
        context.Type = JobType.TimeJob;
        var invoked = false;
    ...
    [Fact]
    public async Task ExecuteTaskAsync_retries_transient_periodic_observation_failures()
    {
        var timeProvider = new FakeTimeProvider();
        var schedulerOptions = new SchedulerOptionsBuilder
        {
    ...
    [Fact]
    public async Task ExecuteTaskAsync_cancels_and_awaits_a_blocked_observation_before_disposal()
    {
        var timeProvider = new FakeTimeProvider();
        var schedulerOptions = new SchedulerOptionsBuilder
        {
