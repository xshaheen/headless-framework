...
Add `Headless.DistributedLocks.Postgres` — the framework's first **cross-node DB-engine** lock backend — built on a new shared `Headless.DistributedLocks.Core.Db` package that both this provider and the future SQL Server provider (#294) consume. Postgres advisory locks are **connection-scoped** (no TTL, no server-stored lock value; the lock lives exactly as long as the holding session or transaction), so this work cannot reuse the existing TTL-based `DistributedLockProvider`. Instead it introduces a connection-scoped provider that maps `pg_advisory_lock`/`pg_advisory_xact_lock` onto the `IDistributedLockProvider` surface, with correctness-grade `HandleLostToken` driven by connection death (`ConnectionMonitor`), optimistic connection multiplexing, PG-native `LISTEN`/`NOTIFY` push wake-up, a durable sequence-backed fencing token, and reader-writer support.
...
flowchart TB
  ...
  subgraph DbCore["Headless.DistributedLocks.Core.Db (new, shared)"]
...
```text
src/
  Headless.DistributedLocks.Core.Db/                 # new shared DB engine
    Headless.DistributedLocks.Core.Db.csproj
    Internal/
  ...
  Headless.DistributedLocks.Postgres/                # new provider
    ...
    README.md
tests/
  Headless.DistributedLocks.Core.Db.Tests.Unit/      # pool + key-agnostic engine logic
...
### Shared DB engine (`Headless.DistributedLocks.Core.Db`)

- R1. New `Headless.DistributedLocks.Core.Db` package holds the provider-agnostic ADO.NET engine; no Npgsql dependency (works against `IDbConnection`/`DbConnection`/`DbDataSource`). (issue R4.1; shared with #294)
...
- **`Core.Db.Tests.Unit`** — covers R10's fence plumbing with a fake `IFencingTokenSource` (handle stamping, null when unregistered, null for RW); the `FencingToken` member itself already ships with its own coverage from #368.
...
### U1. `Core.Db` package scaffold + ADO.NET primitives + `ConnectionMonitor`
...
- **Test suite design:** Create `tests/Headless.DistributedLocks.Core.Db.Tests.Unit` (`Sdk="Headless.NET.Sdk.Test"`) in this unit; cover monitor state transitions against a fake `DbConnection`. Real-socket death fanout deferred to U11 integration. This project also hosts U2's pool tests and U4's fence-seam tests.
- **Test scenarios:**
...
### U4. `IFencingTokenSource` seam in `Core.Db`
...
- **Requirements:** R10
- **Dependencies:** U3
- **Files:** `src/Headless.DistributedLocks.Core.Db/IFencingTokenSource.cs`; wire into `ConnectionScopedDistributedLockProvider.cs` + `ConnectionScopedDistributedLockHandle.cs` (U3).
...
- **Test suite design:** Unit-tested with a fake `IFencingTokenSource` in `Core.Db.Tests.Unit`; PG durable-sequence behavior in U10/U11.
- **Test scenarios:**
...
- **Requirements:** R9
- **Dependencies:** U6
- **Files:** `PostgresReaderWriterLockStorage.cs`, `src/Headless.DistributedLocks.Core.Db/ConnectionScopedReaderWriterLockProvider.cs`, RW overloads in `Setup.cs`.
...
- **Files:** [docs/llms/distributed-locks.md](../../docs/llms/distributed-locks.md), new `src/Headless.DistributedLocks.Postgres/README.md`, new `src/Headless.DistributedLocks.Core.Db/README.md`.
...
In scope: the `Core.Db` shared engine, the Postgres mutex + reader-writer providers, optimistic multiplexing, `ConnectionMonitor`-driven `HandleLostToken`, `LISTEN`/`NOTIFY` wake-up, transaction-coupled static API, `PostgresAdvisoryLockKey`, and the durable fencing sequence that populates the existing `FencingToken`.
...
- **#294 SQL Server provider** — consumes the `Core.Db` engine this PR introduces; adds the `sp_getapplock` strategy + native-block wake-up. This PR must leave the engine provider-agnostic so #294 only adds a strategy + connection wrapper.
...
- **#294** (SQL Server) — shares the `Core.Db` engine; **#364** (fencing tokens) — verified open, but the **interface + best-effort Redis fence already shipped via #368** (`FencingToken` is present on `IDistributedLock`/`LockInfo`/`DistributedLockHandleBase`, with `DistributedLockAcquireResult` carrying it). #364's open remainder is the durable DB sequence (this PR delivers Postgres; #294 delivers SQL Server) + docs.
