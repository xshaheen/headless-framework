...
- **KTD-3. `CacheEntryOptions` is a mutable `sealed class` with an implicit `TimeSpan` conversion, not a record/struct.** Mirrors the existing `CacheOptions`/`InMemoryCacheOptions` shape and accommodates additive growth across M1 PRs (new settable knobs) without changing the type's nature. It is a per-call parameter value, **not** a DI options-pattern type — so no FluentValidation validator; `Duration` is validated inline with `Argument.IsPositive`, exactly as `GetOrAddAsync` validates `expiration` today. Lives in `src/Headless.Caching.Abstractions/Contracts/` beside `CacheOptions`/`CacheValue`.
...
- **Approach:** `[PublicAPI] sealed class CacheEntryOptions` with a settable `Duration` (`TimeSpan`) and `public static implicit operator CacheEntryOptions(TimeSpan duration)`. XML doc states it is the M1 extension point and that fail-safe/timeout/refresh members arrive in later PRs. No validator (KTD-3); callers validate `Duration` at use (see U2/U3).
