...
- KTD6. **Bound new Identity key columns explicitly.** Default newly configured tenant lengths to `DomainConstants.IdMaxLength`; default otherwise unbounded string Identity IDs to 128 within the opt-in. Preserve explicit compatible lengths without estimating provider key budgets or silently shrinking consumer mappings. The EF provider and database enforce physical storage limits; consumers verify their mappings through migrations and representative writes. Keep this policy local to Identity and document its schema impact. Realizes R8, R10, R11.
...
- KTD8. **Consumers own tenant storage semantics.** (session-settled: user-approved on 2026-09-14, replacing the earlier automatic collation and trailing-space policy because metadata ownership should remain portable without imposing database storage formats.) Retain string CLR tenant properties, query filters, concurrency tokens, and explicit Identity constraints. Consumers configure column types, conversions, collations, and tenant-ID validation in their own model and migrations. Headless does not normalize IDs, force collations, add tenant-ID check constraints, or reject providers based on an equality whitelist. Consumers must ensure distinct canonical IDs remain distinct under database equality and storage conversions preserve identity. Governs R3, R5, R6, R10, R11, R12.
...
- Delegate physical key-size validation to the EF provider and database. Preserve the upstream schema-v3 passkey credential primary key and its 1024-byte declared maximum; the existing worst-case SQL Server limitation remains and must be documented. Do not blanket-reject that pre-existing mapping or claim full-length credential support.
...
- Per KTD8, verify that the model preserves consumer storage configuration and adds no tenant collation or tenant-ID check constraints. Exercise persistence without ID normalization. Core conformance uses tenant IDs distinct under each provider's default equality; case-distinct IDs require consumer configuration.
