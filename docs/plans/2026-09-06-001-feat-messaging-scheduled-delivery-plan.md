...
- KD5. **No operator surface.** (session-settled: user-directed — chosen over mirroring the #860 inbox operations: receipts, audit tables across three providers, and a dashboard endpoint are the single largest cost in the feature and buy non-repudiation that a Jobs composition already provides.) Governs R14; see Scope Boundaries. **Superseded by issue #892** (`docs/plans/2026-09-13-1627-feat-messaging-scheduled-delivery-operator-plan.md`): the #860 ledger existing made the marginal cost additive columns and enum values rather than new tables per provider, so Messaging now ships an audited revoke/dispatch-now operator surface for its own scheduled rows.
