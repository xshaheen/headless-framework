...
- KTD8. **Unified bulk-result shape.** Bulk operations return `IReadOnlyList<T>` where each item carries raw blob identity (`Container` + `Path`) plus an optional validated `BlobLocation` and a `Result<…>` (`Headless.Primitives`), so results are correlatable, not positional even when one input path is invalid. A per-item failure does not abort the batch.
