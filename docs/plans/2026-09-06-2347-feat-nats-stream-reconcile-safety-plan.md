...
- **Execution profile:** Branch from and target `main`. Provider-local change in `src/Headless.Messaging.Nats` plus its two test projects and two documentation surfaces.
...
- **CI will not catch a regression in this change.** `make ci-test` runs unit tests only, so every behavior this plan adds is proven by a suite that does not gate merges. The integration suite must be run locally before the change lands, and the Verification Contract treats that as the load-bearing gate rather than a nice-to-have.
...
- The boolean option has no remaining references in `src/`, `tests/`, `demo/`, or documentation.
