...
- A5. Cleanup receipts and cleanup audits default to seven days each. Operator receipts have a 30-day minimum and operator audits default to 90 days. All four durations are positive configurable values. Age is based on each record's immutable creation time; replay does not refresh a receipt.
