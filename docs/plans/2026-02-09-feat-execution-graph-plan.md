...
> **Scope:** Design for tracking parent-child relationships between scheduled jobs, job→message causation chains, and job chaining across both scheduling and messaging subsystems.
>
> **Not in scope:** Fan-out/fan-in patterns (deferred), dashboard/UI for graph visualization (future), OpenTelemetry Activity integration (Full Merge US-032–034), saga/state machine orchestration.
...
2. **Correlation tracing** — Full causation chain from root trigger through all descendant jobs and messages
...
3. **Opt-in complexity** — Chaining is additive. Jobs without parents work exactly as today.
...
5. **Depth limit** — Prevent runaway chains. Configurable but defaulted (max depth 5).
...
- No `MaxDepth` column on the entity. Enforce at the API/manager level via `SchedulingOptions`.
...
US-006–009 (Correlation Propagation) — MUST land first
    ...
    │   │
    │   └── EG-005: IScheduledJobManager extensions
    │
    └── EG-006: ScheduledTrigger hierarchy data
        │
        └── EG-007: Integration tests
            │
            └── EG-008: README + docs
```
...
### EG-004: Child Job Triggering Engine [M]

Core logic: when parent completes, evaluate and trigger child jobs.
...
### EG-005: IScheduledJobManager Extensions [S]
...
**Acceptance Criteria:**
- [ ] `ScheduleChildAsync(name, parentJobName, consumerType, runCondition, payload)` validates: parent exists, depth limit
...
### EG-006: Update ScheduledTrigger with Hierarchy Data [S]
...
### EG-008: Configuration + Documentation [S]

**Files to modify:**
- `src/Headless.Messaging.Core/Scheduling/SchedulingOptions.cs` — add `MaxJobDepth`
...
- [ ] README documents: chaining API, causation headers
- [ ] Code examples for common patterns (A→B chain, A→B on failure)
...
|------|--------|------------|
| Recursive CTE for depth check on every child schedule | Perf hit on deep chains | Cache depth on entity or limit CTE depth to MaxJobDepth |
...
3. **Should `CausationId` on messages published from a job handler be the execution ID or the job name?** Recommendation: execution ID — it's more specific and enables tracing to the exact run.
