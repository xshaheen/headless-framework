...
D14. **Transactional outbox is on by default on the EF-context messaging path, with zero consumer wiring; raw-ADO paths stay explicit opt-in.** When a consumer selects EF storage with `setup.UseEntityFramework<TContext>()`, the EF storage setup auto-registers commit coordination and a DI-registered `IDbContextOptionsConfiguration<TContext>` (EF Core 9+) that attaches the commit-coordination interceptor to the consumer's `DbContext` — even a plain `AddDbContext<TContext>` with no `AddInterceptors(...)`. A `PublishAsync(...)` inside a coordinated transaction then writes its outbox row in the same DB transaction and is discarded on rollback, with no `EnlistCommitCoordination` call to forget. Rationale: the EF path is the one place the framework can reach the consumer's options build and attach the interceptor itself, so the safe behavior is also the default behavior — the prior model (D12's helper-first ergonomics) still required a deliberate enlist that, when forgotten, silently discarded publishes. Opt out with `setup.WithoutTransactionalOutbox()` to restore non-transactional immediate dispatch. A startup gate (`CommitInterceptorStartupGate<TContext>`, `CommitInterceptorProbeMode` Disabled/Warn/Strict, default `Warn`) commits an **empty** transaction (no data mutated) before any hosted service and asserts the interceptor fired; on a mis-wire it logs a loud warning (Warn) or fails startup (Strict opt-in), turning the old "looks transactional but isn't" footgun into a boot-time signal. This is the EF sibling of the SqlServer diagnostic self-probe (D11) — same Warn-default / Strict-opt-in posture. Scope is the EF-context path **only**: the raw-ADO storage paths (`UsePostgreSql(connString)` / `UseSqlServer(connString)`, no `DbContext`) are unchanged and stay explicit opt-in, because there is no `DbContext` to attach an interceptor to, and on-by-default there would start the SqlServer process-wide diagnostic unbidden (D11). Raw paths keep registering `AddPostgreSqlCommitCoordination()`/`AddSqlServerCommitCoordination()` and using the `EnlistCommitCoordination` / `ExecuteCoordinatedTransactionAsync` helpers.
