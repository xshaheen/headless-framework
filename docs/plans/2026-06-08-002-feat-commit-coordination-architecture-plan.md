...
- **Detected-signal fragility.** The SqlServer source depends on `Microsoft.Data.SqlClient` diagnostic event names / property shapes (internal-ish) and can drift across driver upgrades. Mitigation: pin the contract in a single source, add a startup self-probe that asserts the diagnostic fires once on a throwaway commit, and log a loud warning if it does not — fail observable, not silent. The EF interceptor path (preferred where EF is in play) has no such fragility.
