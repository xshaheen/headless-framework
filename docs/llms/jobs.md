...
- **Node identity**: the node owner stamped on job rows is `node@incarnation` (a store-allocated incarnation ID), not `Environment.MachineName`. The node id comes from `CoordinationOptions.ConfiguredNodeId` or the host name `Headless.Core` discovers (`POD_NAMESPACE/POD_NAME`, then the machine name; see [core.md](core.md)), not from `SchedulerOptionsBuilder`.
...
await factory.RunAsync(
    ...
    async (unit, ct) =>
    {
        ...
        // unit.Jobs, not an injected IJobScheduler, for the same reason.
        await unit.Jobs.ScheduleAsync(new OrderReminderRequest(order.Id), reminderDueAt, ct);
    },
    cancellationToken: cancellationToken
);
// Application row, durable message, and job row commit or roll back together.
// IUnitOfWorkFactory.RunAsync(db, ...) (Headless.UnitOfWork.EntityFramework) is the only way to open the
