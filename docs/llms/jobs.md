...
- On the durable path, node identity is `node@incarnation` (store-allocated), not the machine name. Durable job rows are stamped with this owner. On the in-memory single-process path, identity comes from `SchedulerOptionsBuilder.NodeId`, whose default is unique by construction (`{MachineName}:{ProcessId}:{Guid8}`) so co-located processes and same-machine-name K8s pods never collide.
