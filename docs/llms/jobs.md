...
Both types share `BaseJobEntity` (`Id`, `Function`, `Description`, `DateCreated`, `DateUpdated`) and expose `Retries`, `RetryIntervals`, and `OnNodeDeath` policy.
...
- **Entity types**: `TimeJobEntity` / `TimeJobEntity<TTicker>` (parent–child chains), `CronJobEntity`, `CronJobOccurrenceEntity`, and `BaseJobEntity`. New entities keep `Id`, `DateCreated`, and `DateUpdated` unset until a Jobs manager stamps them during `AddAsync` / `AddBatchAsync`.
