...
Relational consumers must apply the Jobs migrations before deploying the new binary, including cancellation; cron pause/time-zone fields; and `ReconciledThroughUtc`, `NextDueUtc`, `MissedRunGraceSeconds`, `OnMissedRun`, nullable `EvaluationFingerprint`, and nullable occurrence `RecoveredFromUtc`. Legacy positions backfill to the uninitialized sentinel, so the first new scheduler wake anchors them at store time instead of replaying history. Quiesce every scheduler node during migration and start only new binaries afterward; mixed versions do not share the atomic materialization contract. The PostgreSQL demos and SQL Server conformance project include reference migrations; custom schemas require equivalent DDL and indexes. Refuse rollback once watermark or recovery state exists unless operators intentionally export or discard that state. Custom persistence providers must implement atomic pause/resume/update plus `GetEarliestCronDispatchCandidatesAsync`, `AdvanceCronScheduleAsync`, and `MaterializeCronScheduleOccurrenceAsync` before upgrading. Selection is uncached and carries store time, position-only advance returns `null` on a lost fence, and materialization always due-fences the exact persisted projection while committing its durable occurrence outcome. Run the shared schedule-position conformance suite for every custom provider.
