...
**Sliding lease for running jobs (#316).** A running job renews its lease on the `SchedulerOptionsBuilder.LeaseRenewalInterval` cadence (≈ `LeaseDuration / 3` by default; an explicit value must be positive and `< LeaseDuration`, validated at startup). Each renewal slides `LockedUntil` forward, fenced on still-owned + non-terminal status; a renewal affecting **zero rows** means the lease was lost and the worker cancels that job's `CancellationToken` (cancel-on-loss, best-effort). Consequences:
