...
- Keyed duplicates retain the first successful create's execution policy, including when a later call supplies explicit overrides. Use generation-fenced replacement to change policy while pending and unclaimed. Pause keyed submissions and replacement during upgrades from the `v1` writer; upgrade all capable processes before resuming. Old code cannot safely resume after a retained `v2` generation is written.
