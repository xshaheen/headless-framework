...
// The block owns the unit, so a retriable failure anywhere in it replays the whole block — publish and
// job write included — with a fresh transaction; neither enlisted write ends replay here.
```
...
Facade calls use those managers internally, so they enlist in the caller's active unit of work (`IUnitOfWorkManager.Current`) and retain the same deferred post-commit dispatch/restart/notification behavior.
