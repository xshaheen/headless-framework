...
4. **Requires `IHttpContextAccessor`.** Reading the `X-Idempotency-Key` header, deriving the cache key from the request path, and writing the `Idempotent-Replayed: true` header all require `HttpContext`. Pulling that into a Mediator behavior defeats the abstraction.
