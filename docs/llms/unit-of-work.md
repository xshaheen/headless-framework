...
- `GetFeature<TFeature>()` is a bridge-package seam, not an application API. Use the typed accessors a bridge ships (`unit.Outbox`, `unit.Jobs`, `unit.AdvisoryLocks`); register a feature (a singleton implementing `IUnitOfWorkFeature`) only when writing such a bridge.
