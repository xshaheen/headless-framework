...
One residual: a mismatch that is visible only to a non-default scheme is rejected by the authorization tier, whose forbid result is collapsed to the generic 404 by `StatusCodesRewriterMiddleware`. A host that does not register that middleware keeps a distinguishable 403 for that narrow case.
