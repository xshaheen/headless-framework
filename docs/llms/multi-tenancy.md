...
When both identifier-based and claim-based resolution are active and a request carries both, the two must resolve to the same canonical tenant. The claim middleware includes a fast-path check (comparing against the preserved per-request identifier-resolution result, never against the ambient `ICurrentTenant.Id`, which its own `Change()` may have already overwritten), but the **authoritative** enforcement point is `TenantIdentifierIntegrityHandler`, an `IAuthorizationHandler` that runs during authorization — after `PolicyEvaluator` has materialized the authenticated principal, which is what makes it correct even for endpoint-scoped or non-default authentication schemes. A mismatch produces the same secure-by-default rejection as an unknown/disabled identifier (byte-identical unless `DetailedResolutionErrors` is on, in which case it is `g:tenant_identifier_mismatch` at 403). Claim-only requests (no identifier resolution happened) are untouched — R19 only applies when both paths ran.
...
- **Tenant catalog** (opt-in; see `Headless.MultiTenancy`'s `Catalog(...)` for wiring):
    ...
    - `TenantResolutionOutcome` / `TenantResolutionKind` — the closed outcome set produced by identifier-based resolution: `Resolved`, `Unknown`, `Disabled`, `Ignored`, `Invalid`
...
- **Tenant catalog** (opt-in; see [Tenant Catalog](#tenant-catalog) for the concepts and extension tiers):
    ...
    - `InMemoryTenantStore` / `UseInMemory(...)` — seeded, immutable snapshot store for tests and small apps; rejects duplicate normalized identifiers or ids at startup.
