...
Custom `IAuthorizationMiddlewareResultHandler` ordering: register app-owned result handlers before `.Authorization(auth => auth.RequireTenant())` when Headless should decorate them. Headless uses the `Headless.Hosting` decoration helper and delegates every non-tenant authorization result to the inner handler. If the app registers or replaces `IAuthorizationMiddlewareResultHandler` after Headless, DI last-registration-wins behavior replaces the Headless tenant mapper. Apps that need their own decorator can use `services.Decorate<IAuthorizationMiddlewareResultHandler, TDecorator>()` after the inner handler is registered.
