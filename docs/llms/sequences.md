...
- Names and partitions compare ordinally and case-sensitively: `"INV"` and `"inv"` are two counters. A name may be at most 128 characters, a partition at most 64, and a tenant id at most 128 (`SequenceFieldLimits`). Longer values, a blank name, and a whitespace-only partition throw `ArgumentException` before any SQL runs. A `null` or empty partition means no partition.
