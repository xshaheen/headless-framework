...
- Per-call tier control (Hybrid; single-tier providers ignore these): `CacheEntryOptions.SkipCacheRead = true` is a force-refresh — `GetOrAddAsync` bypasses the cached read on both tiers and always runs the factory (so no stale fail-safe reserve is read; a factory failure has nothing to fall back to and propagates). `SkipMemoryCacheWrite` / `SkipDistributedCacheWrite` suppress the L1 / L2 write on the factory write-through and `UpsertEntryAsync`; the backplane publish still fires so peers drop stale L1.
- Granular per-tier *read*-skip (Hybrid; single-tier providers ignore these), mirroring FusionCache's `SkipMemoryCacheRead` / `SkipDistributedCacheRead`: `CacheEntryOptions.SkipMemoryCacheRead = true` bypasses the L1 read on `GetOrAddAsync` so the value is served from (or refreshed against) L2 — an existing L2 reserve stays available for fail-safe, and a fresh value read from L2 is still promoted into L1 (promotion is governed by `SkipMemoryCacheWrite`, not the read skip). `SkipDistributedCacheRead = true` bypasses the L2 read so a fresh L1 value serves without an L2 round-trip and an L1 miss falls straight through to the factory. These are the granular counterparts to the coarse `SkipCacheRead`: setting **both** reads neither tier and is equivalent to `SkipCacheRead` (a miss with no reserve), and when `SkipCacheRead` is set it wins outright (the coordinator issues no store read at all, so the two per-tier flags add nothing). The flags are threaded from `CacheEntryOptions` into the store read primitive via a small `FactoryCacheReadOptions` value on `IFactoryCacheStore.TryGetEntryAsync` / `TryGetAllEntriesAsync` (default reads both tiers), so they apply on the factory/coordinator `GetOrAddAsync` read path (including its eager-refresh double-check); the plain `GetAsync` / `GetAllAsync` reads take no `CacheEntryOptions` and always read both tiers.
...
Beyond metrics and traces, every cache exposes a typed, in-process **event surface** via `cache.Events` — native .NET events for consumers that want hooks (warm a related key, invalidate a downstream resource, drive a UI counter) rather than an exporter. Subscribe and unsubscribe with the usual `+=` / `-=`:

```csharp
cache.Events.Hit += (sender, e) => logger.LogDebug("hit {Key} stale={Stale}", e.Key, e.IsStale);
cache.Events.FailSafeActivation += (sender, e) => metrics.FailSafe(e.Trigger);
```
...
**Execution.** Handlers run guarded on a **background task by default** — an exception thrown by a synchronous handler is caught, logged (at `EventHandlerErrorLogLevel`, default `Warning`), and never propagates to the cache caller. Set `AddHeadlessCaching(s => s.SyncHandlers = true)` to run handlers synchronously on the firing thread (deterministic ordering; only for fast handlers). Coordinator-owned events (factory outcome, fail-safe, refresh, and the factory-write `Set`) always dispatch on a background task regardless of `SyncHandlers`, because they are emitted while the per-key factory lock is held — this keeps a same-key re-entrant handler from deadlocking and never runs a handler under the lock. `async void` handlers are **unsupported**: their exceptions bypass the guard (the standard .NET-event footgun) — keep handlers synchronous and start your own guarded fire-and-forget for async side-effects.

**No allocation when unobserved.** An event with no subscriber builds no args and does no work; the coordinator's hot path also short-circuits when no event is subscribed (and no metric/trace listener is attached), so an unobserved cache pays nothing. `ScopedCache<T>` (the key-prefixing wrapper) returns a no-op hub — its shared inner cache would leak other scopes' events and prefixed keys — so subscribe on the underlying (unscoped) cache.
...
- `ICacheEvents` - the typed event surface returned by `ICache.Events` (`cache.Events.Hit += …`): native .NET events for every cache signal (`Hit`/`Miss`/`Set`/`Remove`/`Eviction`/factory/fail-safe/refresh, the bulk `RemoveAll`/`RemoveByPrefix`/`RemoveByTag`/`Clear`/`Flush`, and hybrid `Invalidation`), plus nullable `Memory`/`Distributed` per-tier sub-hubs on the hybrid. Args (`CacheEventArgs` → keyed `CacheKeyEventArgs` and the operation-level variants) carry `CacheName`, `Tier`, and the caller-facing `Key`, with typed enums (`CacheTier`, `CacheEvictionReason`, `CacheFailSafeTrigger`, `CacheFactoryOutcome`, `CacheRefreshKind`, `CacheInvalidationKind`, `CacheInvalidationDirection`). `Events` has a default interface implementation returning the shared no-op `CacheEvents.NoOp`, so existing `ICache` implementers are unaffected. See [Events](#events) for execution semantics and the no-allocation guarantee.
...
- `CacheEventsHub` / `CacheEventsConfig` - the concrete dispatcher behind `ICache.Events` and its execution config. `CacheEventsHub` implements `ICacheEvents`, builds args only when the specific event has a subscriber (no allocation when unobserved), and runs each guarded handler on a background task by default. `CacheEventsConfig` (`SyncHandlers`, `HandlerErrorLogLevel`) carries the execution options, populated from the setup builder's `SyncHandlers` / `EventHandlerErrorLogLevel` (default `Warning`). See [Events](#events).
