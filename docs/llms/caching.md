...
- `ICache` - core interface for cache operations:
    ...
    - Set operations (SetAdd, SetRemove, GetSet). String set members compare with ordinal (case-sensitive) equality on every provider (InMemory matches Redis's byte-exact membership); non-string member equality is provider-native (serialized-byte equality on Redis, default `Equals` on InMemory), so custom `Equals` overrides or non-canonical serializers can diverge across providers.
