...
- `SetExpirationAsync` and `VerifyChecksumAsync` deliberately ignore the request's cancellation token (TusDiskStore parity): tusdotnet invokes them after a PATCH with the request token, which is already cancelled when the client paused/disconnected — exactly when the bookkeeping (sliding-expiration refresh, checksum verify/rollback) must still complete.
