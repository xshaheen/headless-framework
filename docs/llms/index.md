...
- **Caching.** Use `ICache` from `Headless.Caching.Abstractions` for application cache operations. Use `Headless.Caching.Bcl` only when ASP.NET Core Session or another standard integration requires `Microsoft.Extensions.Caching.Distributed.IDistributedCache`. Do not use `IMemoryCache` directly. A framework package never calls `AddHeadlessCaching` itself (it accepts one call per service collection, owned by the host) and never calls `AddMemoryCache()` (that mutates the host's container). Instead it resolves the host's cache **optionally** — `provider.GetService<ICache>()`, hold an `ICache?`, degrade to the source of truth when it is absent — and namespaces its keys by feature (`jobs:`, `messaging:`, `api:`). `Headless.Jobs.EntityFramework`'s cron-expression cache and `Headless.Api.Core`'s `UserAgentParser` (which made its consuming member async precisely to keep this shape — see [api.md](api.md)) are the reference implementations. Reach for a package-private `MemoryCache` only as a last resort — when the consumption point cannot be made asynchronous (so the async `ICache` genuinely cannot be called) and the value is local CPU work a remote round trip would not pay for — and even then own the instance rather than registering the shared `IMemoryCache`; `MessagingDashboardCache` in [messaging.md](messaging.md) is the sole example.
