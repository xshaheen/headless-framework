...
- **Caching.** Use `ICache` from `Headless.Caching.Abstractions` for application cache operations. Use `Headless.Caching.Bcl` only when ASP.NET Core Session or another standard integration requires `Microsoft.Extensions.Caching.Distributed.IDistributedCache`. Application code must not use `IMemoryCache` directly. A framework package never calls `AddHeadlessCaching` itself (it accepts one call per service collection, owned by the host) and never calls `AddMemoryCache()` or injects the shared `IMemoryCache` (both make package entries compete for the host application's cache budget). For optional source-of-truth caching, resolve the host's `ICache` with `provider.GetService<ICache>()`, hold an `ICache?`, degrade when absent, and namespace keys by feature; `Headless.Jobs.EntityFramework`'s cron-expression cache is the reference implementation. For bounded, process-local memoization of derived CPU work where a remote round trip is counterproductive or values should not leave the process, own and dispose a private `MemoryCache`; `Headless.Api.Core`'s `UserAgentParser` and `MessagingDashboardCache` are the reference implementations. See [api.md](api.md) and [messaging.md](messaging.md).
...
- `Headless.Messaging.Bus.Abstractions` — Broadcast publisher contract: `IBus` with per-call delivery modes.
- `Headless.Messaging.Queue.Abstractions` — Point-to-point publisher contract: `IQueue` with per-call delivery modes.
