...
- **Caching.** Use `ICache` from `Headless.Caching.Abstractions` for application cache operations. Use `Headless.Caching.Bcl` only when ASP.NET Core Session or another standard integration requires `Microsoft.Extensions.Caching.Distributed.IDistributedCache`. Do not use `IMemoryCache` directly. A framework package that needs private in-process memoization and cannot take a `Headless.Caching` dependency (`AddHeadlessCaching` allows only one call per service collection) owns a dedicated cache type instead of registering the shared `IMemoryCache` — see `MessagingDashboardCache` in [messaging.md](messaging.md).
