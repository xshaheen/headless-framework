...
- Address every blob with `BlobLocation(container, path)`. The constructor validates path security (traversal, control characters, absolute paths, the reserved sidecar suffix) once, so pass the value through — do not pre-normalize. The `params string[]` convenience constructor joins segments with `/`: `new BlobLocation("uploads", "images", fileName)` is the key `images/<fileName>` in container `uploads`. `Move`/`Copy` take a source and a destination `BlobLocation`.
...
- List with `ListAsync(BlobQuery) → BlobPage(Items, ContinuationToken?)`. A `null` token marks the last page; otherwise round-trip the opaque token into a new `BlobQuery` to fetch the next page. Prefer the `GetBlobsAsync(BlobQuery)` streaming extension (an `IAsyncEnumerable<BlobInfo>`) for full enumeration, or `GetBlobsListAsync(query, limit)` to materialize.
- `BlobQuery.Prefix` is the only filter pushed to the backend (and it is validated through the same seam as `BlobLocation`, so a `../` prefix can never reach enumeration). Glob (`*`, `?`) is a **client-side** extension layered over streaming: `GetBlobsAsync(query, globPattern)`. `DeleteAllAsync(BlobQuery)` deletes by validated prefix; glob-delete is list + filter + bulk-delete.
...
- All six providers support metadata, typed as `IReadOnlyDictionary<string, string>?` with non-null values, consistently on the `UploadAsync` parameter, `BlobInfo.Metadata`, and `BlobDownloadResult.Metadata`. FileSystem and SFTP store it in a **sidecar companion file** (reserved `.hlmeta` suffix) beside the blob; sidecars are filtered from every listing/exists/delete result, and a blob key ending in the reserved suffix is rejected at `BlobLocation` construction. Some enumeration responses omit per-object metadata (S3 list API, SFTP) — `BlobInfo.Metadata` may be `null` in a listing even though `GetBlobInfoAsync` returns it.
- `MoveAsync(source, destination)` is **non-atomic**: copy-then-delete with a best-effort rollback (delete the destination copy) if the source delete fails. `CopyAsync` leaves the source intact. On FileSystem/SSH the sidecar moves with the blob.
- Bulk operations return `IReadOnlyList<BlobBulkResult>` — each item carries the blob's `BlobLocation` and a `Result<bool, Exception>` (`Ok(true)` success, `Ok(false)` not-found for delete, `Fail(ex)` on error). Correlate by identity, not position; a per-entry failure does not abort the batch.
...
`BlobQuery.Prefix` is the only filter pushed down to the backend, and it is path-security validated at construction. Glob matching (`*`, `?`) is an explicit **client-side** filter through one shared matcher, layered over streaming: `GetBlobsAsync(query, globPattern)`. This ends the per-provider regex divergence and is honest about which filtering the server does (prefix) versus the client (glob). `DeleteAllAsync(BlobQuery)` deletes every blob matched by the validated prefix and returns the count; a glob-scoped delete is list + filter + bulk-delete in the consumer.
...
Sidecar trade-offs the agent must know: write order is content-first then sidecar, and a missing sidecar reads as empty metadata, so reads stay safe across a crash window — but the pair is **non-atomic** on FileSystem/SFTP (no transaction). Sidecars are filtered from every listing, existence, count, and delete-all result, so they never surface as blobs or match a prefix/glob. Deleting a blob removes its sidecar, so re-uploading the same key without metadata cannot resurrect stale metadata. A blob key that would collide with the reserved `.hlmeta` form is rejected at `BlobLocation` construction. Note that the S3 list API and SFTP omit per-object metadata from enumeration responses, so `BlobInfo.Metadata` may be `null` in a listing even though `GetBlobInfoAsync` returns it.

### Move, copy, and bulk results

Object stores have no native rename, so `MoveAsync(source, destination)` is a copy-then-delete and is documented as non-atomic: if the source delete fails after a successful copy, the implementation makes a best-effort attempt to roll back by deleting the destination copy so the original is preserved. `CopyAsync(source, destination)` leaves the source intact. Both take source and destination `BlobLocation` values and may cross containers within a single store. On FileSystem/SFTP the metadata sidecar moves with the blob.

Bulk operations return `IReadOnlyList<BlobBulkResult>`, where each item pairs the input `BlobLocation` with a `Result<bool, Exception>` (`Headless.Primitives`), so results are correlated by identity rather than by position. For `BulkUploadAsync`: `Ok(true)` on success, `Fail(ex)` on failure. For `BulkDeleteAsync`: `Ok(true)` deleted, `Ok(false)` not found, `Fail(ex)` on failure. A per-entry failure does not abort the batch.
...
- **Upload no longer auto-creates the container.** Uploading to a missing bucket/container now throws; call `IBlobContainerManager.EnsureContainerAsync` (or provision via IaC) first. The `AutoCreateContainer` options remain for option-shape compatibility but are ignored by the write path.
...
- `BlobBulkResult` — identity-carrying bulk outcome (`BlobLocation` + `Result<bool, Exception>`).
