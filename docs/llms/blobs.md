---
domain: Blob Storage
packages: Blobs.Abstractions, Blobs.Core, Blobs.Aws, Blobs.Azure, Blobs.CloudflareR2, Blobs.FileSystem, Blobs.Redis, Blobs.SshNet, Api.Blobs
---
...
- **Presigned URLs without a cloud provider**: `Headless.Api.Blobs` — a signed download/upload endpoint that gives FileSystem, Redis, and SFTP stores `IPresignedUrlBlobStorage`, streaming the bytes through the application.
...
- Presigned URLs are a separate capability discovered with an `is`-cast from the store (`storage is IPresignedUrlBlobStorage presigned`) and take a `BlobLocation`. AWS, Azure, and CloudflareR2 support it natively; FileSystem, Redis, and SshNet support it only after `setup.UseSignedUrlEndpoint(...)` from `Headless.Api.Blobs` plus `app.MapBlobSignedUrlEndpoint()`, which serve the bytes through the application. For **named** stores a keyed `IPresignedUrlBlobStorage` is registered too (resolve via `[FromKeyedServices("name")]`). There is no global (unkeyed) `IPresignedUrlBlobStorage` registration — do not inject it without a key.
- Upload URLs take an optional `PresignedUploadConstraints` (`ContentType`, `MaxLength`). A backend that cannot enforce a constraint throws `NotSupportedException` instead of ignoring it: S3 and R2 sign the content type but refuse `MaxLength`, Azure refuses both, and the signed-URL endpoint enforces both. Code that must run on every provider validates the uploaded blob after upload instead.
...
The token is a **serializable opaque string** that survives a web-request boundary (unlike the previous closure-backed cursor). It is provider-specific; callers must not parse, compare, or persist it across a provider change. Because the token round-trips through untrusted boundaries (a web pagination cursor), every provider wraps its native cursor in one shared opaque envelope: a malformed/forged token fails uniformly with a catchable `ArgumentException` ("not a valid opaque token produced by this provider") instead of leaking a raw backend SDK exception; Redis additionally rejects a decoded-but-non-numeric cursor with the same error. The honest residual: a syntactically valid token from a different provider or store is indistinguishable from a real one, so its result is backend-defined — typically an empty page or a backend error. Stability is documented as explicit tiers:

| Tier | Providers | Token encoding (inside the shared envelope) | Stability |
| --- | --- | --- | --- |
| Server-side stable | AWS S3, Cloudflare R2, Azure | native continuation token | stable across concurrent writes |
| Emulated re-scan | FileSystem, SFTP | lexicographic start-after-key | weaker stability under concurrent writes; each page re-scans from the start (full enumeration is O(n²/pageSize) I/O) |
...
Whether that provisioning step is required at all is a data-plane fact, so it lives on the store itself: `IBlobStorage.RequiresContainerProvisioning` is `true` for AWS, Azure, CloudflareR2, FileSystem, and SSH (write to a never-provisioned container throws) and `false` for Redis (the backing hash is created lazily on first write, so no provisioning step exists). Infrastructure consumers combine the two: a `null` manager is only a problem when the store reports `RequiresContainerProvisioning == true` — the DataProtection `PersistKeysToBlobStorage` guardrail is the reference example.
...
| `Headless.Blobs.SshNet` | Files must land on a remote SFTP/SSH server or legacy system | High-throughput or presigned-URL workloads | Slower; presigned URLs only through the `Headless.Api.Blobs` endpoint, which streams through the application; sidecar metadata costs a second round-trip; opens live SSH connections |
...
- `IPresignedUrlBlobStorage` — optional presigned GET + PUT URL capability over a `BlobLocation`; implemented natively by AWS, Azure, and CloudflareR2, and for the other providers by the `Headless.Api.Blobs` decorator.
- `PresignedUploadConstraints` — optional `ContentType` and `MaxLength` an upload URL imposes; a backend that cannot enforce one throws `NotSupportedException`.
...
- Presigned download/upload URLs over a `BlobLocation` via `IPresignedUrlBlobStorage` (named stores only; feature-detect via cast for the default store). An upload `ContentType` constraint is signed into the URL, so the uploader must send that exact `Content-Type` header; a `MaxLength` constraint throws `NotSupportedException` because a presigned PUT cannot bound its size.
...
- Presigned download/upload URLs over a `BlobLocation` via `IPresignedUrlBlobStorage` (SAS-based; named stores only — feature-detect via cast for the default store). A SAS cannot constrain an upload, so any `PresignedUploadConstraints` restriction throws `NotSupportedException`.
...
- No native presigned URL support. `IPresignedUrlBlobStorage` is registered for FileSystem stores only by `UseSignedUrlEndpoint` from `Headless.Api.Blobs`.
...
- No native presigned URL support. `IPresignedUrlBlobStorage` is registered for Redis stores only by `UseSignedUrlEndpoint` from `Headless.Api.Blobs`.
...
- No native presigned URL support. `IPresignedUrlBlobStorage` is registered for SshNet stores only by `UseSignedUrlEndpoint` from `Headless.Api.Blobs`.
...
- **Download.** `GET` streams the blob with range support when the stream is seekable. These stores do not keep a content type, so the response type is derived from the key's extension. Responses carry `X-Content-Type-Options: nosniff` and `Content-Security-Policy: sandbox`, because the bytes come from the application's own origin, where an uploaded HTML or SVG file could otherwise run script. For untrusted uploads, prefer a separate host name for `BaseUrl`.
- **Upload.** `PUT` writes the request body through `UploadAsync` and returns 200. A `ContentType` constraint rejects any other media type with 415 (parameters such as `charset` are ignored). A `MaxLength` constraint requires `Content-Length` (411 without it) and rejects a larger body with 413 before any byte reaches the store; it also sets the server's request-body limit to that length. Without `MaxLength`, the server's default body limit applies. A missing top-level container is a server error, as for `UploadAsync`.
