...
- Use the `Headless.IO` stream helpers (`stream.GetAllBytesAsync()`, `stream.GetAllText()`, `FileHelper.ReadAllTextAsync(...)`) rather than hand-rolling buffers — they avoid async-over-sync, pre-size buffers when the length is known, and retry transient `IOException`s.
