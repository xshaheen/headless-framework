...
- When implementing a custom `IDistributedLock`, `IDistributedReadWriteLock`, or `IDistributedSemaphoreProvider`, expose `TimeProvider` and `Logger` (the composite coordinator reads both off the provider) and preserve ordinal resource identity. If the backend aliases ordinal-distinct names, reject non-canonical names or require callers to canonicalize them before composite acquisition; normalizing only inside the provider is too late.
