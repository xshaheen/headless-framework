...
- **Migration note — the "never publish outside a transaction" guardrail is gone.** A host that set `MessagingOptions.DefaultEnlistment = TransactionEnlistment.Required`, or registered a type `WithEnlistment(TransactionEnlistment.Required)`, used it to make an un-enlisted publish fail loudly. Both members are deleted, so that host gets a compile error, not a silent behavior change. Nothing replaces it as a host-wide setting: the guarantee is now structural per call site. Reading `unit.Outbox.PublishAsync(...)` proves enlistment at the line, and an `IBus.PublishAsync` in code that must be transactional is a review finding rather than a runtime throw. Where a build-time guard is wanted, ban the `IBus`/`IQueue` *types* in the transactional assembly (a banned-API analyzer entry, or an architecture test) rather than the package reference: `Headless.Messaging.UnitOfWork` references both abstraction packages, so those types are on the compile surface of anything that uses the outbox.
- **An enlisted publish you write forfeits execution-strategy replay from that line onward.** The durable row is written outside EF's change tracker, so re-running the unit after a transient failure would duplicate the message; the publish therefore calls `IUnitOfWork.PreventRetry()` before it writes. Inside `unitOfWorkManager.RunAsync(db, …)` this means: a retriable failure *before* the first enlisted publish replays the whole block; a failure after it is surfaced without replay. This was a narrow consequence when enlistment was opportunistic; it is now the ordinary cost of the enlisted path.
- **The entity-emitted integration-event path is the exemption, and it keeps replay.** `PreventRetry()` is skipped when the publish carries `MessageOptions.IsRetainedForTransactionReplay`, which is `internal` and set only by `OutboxIntegrationEventDispatcher` — the bridge that publishes the integration events an entity emitted during `SaveChangesAsync`. The save pipeline retains those captured occurrences and re-publishes them on a replayed attempt, so a `HeadlessDbContext` save whose only enlisted publishes came from emitted integration events stays retriable under `EnableRetryOnFailure`. Application code cannot set the flag, so it cannot opt its own publishes into this. A save that mixes both — emitted integration events plus a direct `unit.Outbox` publish in a handler or in the surrounding block — is not retriable, because the direct publish marked the unit.
...
- **Jobs is the exception that keeps the enum.** `TransactionEnlistment` still lives in `Headless.UnitOfWork.Abstractions` and still drives `JobOptions.Enlistment` / `RecurringJobOptions.Enlistment` with its own guarantee matrix and strictest-wins composition — unchanged by this refactor. It no longer has anything to do with Messaging. See [Enlisted Enqueue](jobs.md#enlisted-enqueue-atomic-enqueue).
...
- A publish made through this surface forfeits EF Core execution-strategy replay for the rest of the unit: the durable row is written outside the change tracker, so re-running the unit after a transient failure would duplicate the message. The publish calls `IUnitOfWork.PreventRetry()` before writing. This holds for every publish application code can write here — the one exemption is keyed on an `internal` option the EF integration-event dispatcher sets for occurrences its save pipeline can reproduce, and no consumer can set it.
...
- Direct publishing bypasses storage entirely, while delayed delivery is always durable. `IBus`/`IQueue` are registered as singletons here (`TryAddSingleton`) and never consult a unit of work; `AddHeadlessMessaging` also registers the unit-of-work feature provider that backs `unit.Outbox`.
- An enlisted publish calls `IUnitOfWork.PreventRetry()` before the write, so the unit is no longer replayable by an EF execution strategy and a fresh unit of work is required after a transaction failure. The EF integration-event bridge exempts only captured occurrences that its save pipeline retains for replay.
