...
**Callbacks are at-least-once — make response consumers idempotent.** The response rides the durable bus path. The outbox write and the request's success mark are not a single transaction, so a crash between them redelivers the request and republishes the response. Deduplicate on `CorrelationId` in the response consumer; the framework does not deduplicate callback deliveries. Multi-hop chains (`context.SetNextCallback(...)`) multiply this — every intermediate consumer must be idempotent too.
