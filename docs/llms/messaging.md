...
`NatsMessagingOptions.StreamProvisioning` decides what consumer startup does about that stream. `Verify` (the default) creates a missing stream but throws with the divergent fields rather than writing to one that already exists; `Reconcile` updates an existing stream and restores the behavior of the removed `EnableSubscriberClientStreamAndSubjectCreation` flag's `true`; `Disabled` neither creates nor modifies. The default changed because the old flag's `true` silently overwrote the storage class, replicas, and limits of a stream provisioned with the NATS CLI, Terraform, or a Kubernetes operator on every startup.
