...
- [Headless.Sms.Twilio](#headlesssmstwilio)
  ...
  - [Configuration](#configuration-6)
    ...
    - [Code Configuration](#code-configuration-1)
...
- [Headless.Sms.Vodafone](#headlesssmsvodafone)
  ...
  - [Configuration](#configuration-8)
    - [appsettings.json](#appsettingsjson-6)
...
Install `Headless.Sms.Abstractions` + exactly one provider package:
- **International**: `Sms.Twilio` (most popular), `Sms.Aws` (AWS SNS), `Sms.Infobip` (global platform)
- **MENA regional**: `Sms.Cequens`, `Sms.Connekio`, `Sms.VictoryLink`, `Sms.Vodafone`
- **Development**: `Sms.Dev` (logs to file or silently discards)

All providers register `ISmsSender` as singleton. Code against `ISmsSender` and `SendSingleSmsRequest`/`SendSingleSmsResponse` from Abstractions. Swap providers by changing DI registration only.

## Agent Instructions

- Always use `Sms.Dev` in development/test environments to avoid sending real SMS. Use `AddDevSmsSender("sms-log.txt")` for file logging or `AddNoopSmsSender()` for silent discard.
- Code against `ISmsSender` from Abstractions, never against provider-specific types.
- Each provider has its own `Add*SmsSender()` extension method (e.g., `AddTwilioSmsSender`, `AddAwsSnsSmsSender`, `AddCequensSmsSender`).
- Check `response.IsSuccess` after `SendAsync()` -- providers return structured errors via `SendSingleSmsResponse.ErrorMessage`.
- Only one provider can be registered at a time (all register as `ISmsSender` singleton).
- For Twilio, you can use either `From` (phone number) or `MessagingServiceSid` -- not both.
- For AWS SNS, you need AWS SDK credentials configured separately via `GetAWSOptions()`.
- Regional providers (Cequens, Connekio, VictoryLink, Vodafone) typically require vendor-issued API credentials.
