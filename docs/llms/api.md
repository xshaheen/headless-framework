...
- `IUserAgentParser` — `GetDeviceInfoAsync(userAgent, ct)`, the User-Agent → `"Windows Chrome"` parse behind `IWebClientInfoProvider.GetDeviceInfoAsync` (implemented in `Headless.Api.Core` over DeviceDetector.NET). Substitute it to stub device detection in tests, or to swap the parser entirely. The default implementation memoizes in the host's registered `ICache` under the feature-namespaced `api:user-agent:` key prefix — the same optional-cache contract as `Headless.Jobs.EntityFramework`'s cron cache: it resolves `ICache` with `GetService`, so parses are cached when the host has registered a `Headless.Caching` provider and run directly on every call when it has not. Negatives (unidentifiable agents) are cached too, so garbage is parsed at most once per TTL. `UserAgentParserOptions`: `SlidingExpiration` 6h, `Duration` (absolute cap) 24h, `MaxUserAgentLength` 512 (longer values truncated before parsing and keying). It is asynchronous precisely so the memo can cross an out-of-process tier; it never registers the shared `IMemoryCache` and owns no private cache.
