...
- All amounts in CashIn are in **cents** (multiply by 100). CashOut uses decimal amounts directly.
...
- CashIn and CashOut have **separate** API keys and credentials. CashOut also requires username/password.
- CashIn uses Paymob's v2 Payment Intentions API; prefer `CreateIntentionAsync` over legacy order flows.
- All brokers are registered as **singletons** — do not create scoped/transient registrations.
