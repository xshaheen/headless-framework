...
This mirrors precedent cited when the redesign was decided: MassTransit's `ScopedConsumeContextProvider` and Wolverine's `ScopedMessageContextHolder` (which Wolverine adopted specifically to fix GH-2583/GH-3001 — the same class of ambient-propagation bug) both moved from `AsyncLocal`/ambient flow to a scoped field for the same reason. See [Unit of Work](../../llms/unit-of-work.md) for the resulting contract.
