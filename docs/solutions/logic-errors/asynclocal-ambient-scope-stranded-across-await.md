...
The ambient `AsyncLocal`-based commit-coordination design (`Headless.CommitCoordination.*`, the whole subject of this entry) has since been replaced twice. The first replacement, `Headless.UnitOfWork.*` as a **scoped** design, kept a `Current` field on a scoped `IUnitOfWorkManager` with no `AsyncLocal` anywhere, which fixed this bug structurally — there was no async-flow propagation left to lose. The current design goes one step further and has **no ambient at all**: `IUnitOfWorkFactory` is a singleton that keeps no record of the units it opens, the `IUnitOfWork` handle is the unit's only identity, and code reaches a unit by being handed the handle, by reading it off the object the unit was begun on (`db.UnitOfWork()`, `connection.UnitOfWork()`), or by joining it through `RunAsync(db, …)` / `RunAsync(connection, …)` on that same object. Either way, the capture-before-await discipline this bug exists to teach is no longer a discipline application code or provider code has to hold.

The scoped step mirrored precedent cited when the redesign was decided: MassTransit's `ScopedConsumeContextProvider` and Wolverine's `ScopedMessageContextHolder` (which Wolverine adopted specifically to fix GH-2583/GH-3001 — the same class of ambient-propagation bug) both moved from `AsyncLocal`/ambient flow to a scoped field for the same reason. See [Unit of Work](../../llms/unit-of-work.md) for the resulting contract and for why the scoped field was retired as well.
