...
- **Native model** — `Headless.DistributedLocks` and `Headless.Jobs` emit `Activity`/`Meter` directly through BCL `System.Diagnostics` primitives from their Core packages; consumers subscribe by name (`AddMeter("Headless.DistributedLocks")` / `AddSource(...)`) or the typed `Add<Feature>Instrumentation()` helpers with no further opt-in — subscribing IS the activation. (Jobs briefly carried a two-step activation — a logger-only default `IJobsInstrumentation` plus an `AddOpenTelemetryInstrumentation()` swap — unified to single opt-in in #695.) No `OpenTelemetry.*` dependency in either feature package.
