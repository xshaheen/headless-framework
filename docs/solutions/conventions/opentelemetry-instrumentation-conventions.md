...
- **Native model** — `Headless.DistributedLocks` emits `Activity`/`Meter` directly through BCL `System.Diagnostics` primitives from its Core package; consumers subscribe by name (`AddMeter("Headless.DistributedLocks")` / `AddSource(...)`) with no further opt-in. `Headless.Jobs` also emits natively from `Jobs.Core`, but activation is two-step: the default `IJobsInstrumentation` is a logger-only implementation that never starts activities, so a consumer must first call `AddOpenTelemetryInstrumentation()` to swap in the `ActivitySource`-based implementation, then subscribe by name or via the typed `AddJobsInstrumentation()` helper. No `OpenTelemetry.*` dependency in either feature package.
...
- Deciding whether a subsystem needs a `.OpenTelemetry` satellite package — the answer is no, unconditionally; even messaging's has been removed in favor of native emission.
