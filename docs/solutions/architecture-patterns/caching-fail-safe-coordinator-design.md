...
### 10. Detached background completion needs a write gate and an optional ceiling
...
The lock hand-off protects the key only while the background task is still under coordinator ownership. `BackgroundFactoryCeiling` is opt-in and defaults to `Timeout.InfiniteTimeSpan` (no ceiling), matching comparable caches (FusionCache, Caffeine, sturdyc), which all let the detached refresh run to completion. When a consumer sets a finite ceiling, it races the factory and releases the per-key lock if a token-ignoring factory does not finish. On the ceiling branch, cancel the internal token and do **not** await the factory. A non-cooperative factory may continue running untracked, but the success write must be gated on the internal token so an abandoned factory cannot clobber a newer timeout-path value after the lock is released. With the default infinite ceiling, the coordinator skips the race entirely and observes the factory to completion, still gating the late write on the internal token.
