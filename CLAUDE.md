...
- `IUnitOfWorkManager` is a scoped DI service whose `Current` is a plain field, never an `AsyncLocal`, and the developer opens a unit of work explicitly. The three Jobs manager interfaces are scoped facades that read `Current` and enlist per `TransactionEnlistment`, so a singleton or hosted service that needs one creates its own scope. Messaging does not work that way: `IBus`/`IQueue` are autonomous singletons that never read `Current`, and an enlisted publish goes through `unit.Outbox` (`Headless.Messaging.UnitOfWork`), which always writes inside the unit's transaction, refuses when the storage cannot join it, and calls `PreventRetry()` so the unit gives up EF execution-strategy replay from that line on — skipped only for the EF integration-event dispatcher's own publishes, which carry an `internal` retained-for-replay flag and keep the save retriable. The full contract, including join-by-default nesting and the `IUnitOfWorkFeatureProvider` seam behind `unit.Outbox`, is in [docs/llms/unit-of-work.md](docs/llms/unit-of-work.md). (2026-09-20)
