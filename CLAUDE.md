...
- A namespace that exactly matches another package's name belongs to that package (only `Headless.Api.Abstractions` may ship the `Headless.Api.Abstractions` namespace); outside that, family packages share their feature root freely.
